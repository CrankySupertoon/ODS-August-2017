# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.battle.BattleBase
...
MINIGAME_PERCENTAGE = 0.05
CLIENT_INPUT_TIMEOUT = config.GetFloat('battle-input-timeout', TTLocalizer.BBbattleInputTimeout)
CLIENT_MINIGAME_TIMEOUT = config.GetFloat('minigame-battle-input-timeout', TTLocalizer.BBminigameInputTimeout)
MIN_MINIGAME_WIN = 1
MAX_MINIGAME_WIN = 8
MINIGAME_WIN_THRESHOLD = 10
MINIGAME_MIN_MAX_HP = 30.0
MINIGAME_MAX_HITS = 3
MINIGAME_SQUISH_THRESHOLD = 40.0

def getMinigameSquish(hp):
    return int(math.ceil(hp / MINIGAME_SQUISH_THRESHOLD))
...
def getDefaultSuitAttacks():
    return [ getDefaultSuitAttack() for x in xrange(4) ]
...
def getDefaultSpecialSuitAttacks():
    return [ getDefaultSpecialSuitAttack() for x in xrange(4) ]


def getDefaultSpecialSuitAttack():
    return [NO_ID, NO_ATTACK, 0]
...
SERVER_MINIGAME_TIMEOUT = CLIENT_MINIGAME_TIMEOUT + SERVER_BUFFER_TIME
    ...
    def buildJoinPointList(self, avPos, destPos, toon = 0):
        ...
        if dist < minDist:
            ...
            if toon == 1:
                ...
                elif nearestP in BattleBase.toonCwise:
            ...
            elif nearestP in BattleBase.suitCwise:
                self.notify.debug('buildJoinPointList() - clockwise')
