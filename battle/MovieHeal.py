# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.battle.MovieHeal
from panda3d.core import Camera, GeomNode, NodePath, Point3, VBase4, Vec3, headsUp
...
def __healTickle(heal, hasInteractivePropHealBonus):
    ...
    def scaleFeather(feather, toon = toon, target = target):
        toon.pose('tickle', 63)
        toon.update(0)
        hand = toon.getRightHand()
    ...
    featherTrack = Parallel(ActorInterval(feather, 'feather'), Sequence(Wait(tFeatherScaleUp), Func(feather.reparentTo, hand), Func(scaleFeather, feather), LerpScaleInterval(feather, dFeatherScaleUp, feather.getScale, MovieUtil.PNT3_NEARZERO)), Sequence(Wait(toon.getDuration('tickle') - dFeatherScaleDown), LerpScaleInterval(feather, dFeatherScaleDown, MovieUtil.PNT3_NEARZERO, None)))
    tHeal = 3.0
    ...
    track.append(mtrack)
    track.append(Func(MovieUtil.removeProp, feather))
...
def __healJoke(heal, hasInteractivePropHealBonus):
    ...
    hand = toon.getRightHand()
    dMegaphoneScale = 0.5
    tracks.append(Sequence(Wait(tDoSoundAnimation), Func(megaphone.reparentTo, hand), LerpScaleInterval(megaphone, dMegaphoneScale, MovieUtil.PNT3_ONE, MovieUtil.PNT3_NEARZERO), Wait(toon.getDuration('sound') - 2.0 * dMegaphoneScale), LerpScaleInterval(megaphone, dMegaphoneScale, MovieUtil.PNT3_NEARZERO, MovieUtil.PNT3_ONE), Func(MovieUtil.removeProp, megaphone)))
...
def __healSmooch(heal, hasInteractivePropHealBonus):
    ...
    hand = toon.getRightHand()
    dScale = 0.5
    lipstickTrack = Sequence(Func(MovieUtil.showProp, lipstick, hand, Point3(-0.27, -0.24, -0.95), Point3(-118, -10.6, -25.9)), LerpScaleInterval(lipstick, dScale, MovieUtil.PNT3_ONE, MovieUtil.PNT3_NEARZERO), Wait(toon.getDuration('smooch') - 2.0 * dScale), LerpScaleInterval(lipstick, dScale, MovieUtil.PNT3_NEARZERO, MovieUtil.PNT3_ONE), Func(MovieUtil.removeProp, lipstick))
...
def __healDance(heal, hasInteractivePropHealBonus):
    ...
    leftHand = toon.getLeftHand()
    rightHand = toon.getRightHand()
    dScale = 0.5
    propTrack = Sequence(Func(MovieUtil.showProp, hat, rightHand, Point3(0.23, 0.09, 0.69), Point3(180, 0, 0)), Func(MovieUtil.showProp, cane, leftHand, Point3(-0.28, 0.0, 0.14), Point3(0.0, 0.0, -150.0)), MovieUtil.getScaleIntervals([hat, cane], dScale, MovieUtil.PNT3_NEARZERO, MovieUtil.PNT3_ONE), Wait(toon.getDuration('happy-dance') - 2.0 * dScale), MovieUtil.getScaleIntervals([hat, cane], dScale, MovieUtil.PNT3_ONE, MovieUtil.PNT3_NEARZERO), Func(MovieUtil.removeProp, hat), Func(MovieUtil.removeProp, cane))
...
def __healJuggle(heal, hasInteractivePropHealBonus):
    ...
    hip = toon.getPart('legs').find('**/joint_hips')
    cubeTrack = Sequence(Func(cube.reparentTo, hip), ActorInterval(cube, 'cubes'), Func(MovieUtil.removeProp, cube))
