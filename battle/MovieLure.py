# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.battle.MovieLure
from panda3d.core import Camera, Plane, PlaneNode, Point3, Point4, Vec3, Vec4
...
def __createFishingPoleMultiTrack(lure, dollar, dollarName):
    ...
    poleTrack = Sequence(Func(pole.reparentTo, hand), ActorInterval(pole, 'fishing-pole'), Func(MovieUtil.removeProp, pole))
...
def __createMagnetMultiTrack(lure, magnet, pos, hpr, scale, isSmallMagnet = 1, npcs = []):
    ...
    magnetTrack = Sequence(Wait(0.7), Func(MovieUtil.showProp, magnet, hand, pos, hpr, scale), Wait(6.3), Func(MovieUtil.removeProp, magnet))
...
def __createHypnoGogglesMultiTrack(lure, npcs = []):
    ...
    hand = toon.getLeftHand()
    gogglesTrack = Sequence(Wait(0.6), Func(MovieUtil.showProp, goggles, hand, pos, hpr, scale), ActorInterval(goggles, 'hypno-goggles', duration=2.2), Func(MovieUtil.removeProp, goggles))
    ...
    for target in targets:
        ...
        if sidestep == 0:
            ...
            if kbbonus == 1 or hp > 0:
                ...
                suitTrack.append(Func(suit.loop, 'neutral'))
                suitTrack.append(Wait(suitDelay))
                suitTrack.append(ActorInterval(suit, 'hypnotize', duration=3.1))
                suitTrack.append(Func(suit.setPos, battle, reachPos))
                suitTrack.append(Func(suit.loop, 'lured'))
                suitTrack.append(Func(battle.lureSuit, suit))
                if hp > 0:
...
def __createSuitDamageTrack(battle, suit, hp, lure, trapProp):
    if trapProp is None or trapProp.isEmpty():
        ...
        if trapName == 'banana':
            ...
            damageTrack = Sequence(Wait(0.5), Func(suit.showHpText, -hp, openEnded=0, attackTrack=TRAP_TRACK, attackLevel=0), Func(suit.updateHealthBar, hp))
            ...
            suitTrack = ActorInterval(suit, 'rake', duration=rakeAnimDuration)
            damageTrack = Sequence(Wait(0.5), Func(suit.showHpText, -hp, openEnded=0, attackTrack=TRAP_TRACK, attackLevel=1), Func(suit.updateHealthBar, hp))
            ...
            suitTrack = ActorInterval(suit, 'slip-backward')
            damageTrack = Sequence(Wait(0.5), Func(suit.showHpText, -hp, openEnded=0, attackTrack=TRAP_TRACK, attackLevel=2), Func(suit.updateHealthBar, hp))
            soundTrack = Sequence(SoundInterval(globalBattleSoundCache.getSound('AA_pie_throw_only.ogg'), duration=0.55, node=suit), SoundInterval(globalBattleSoundCache.getSound('Toon_bodyfall_synergy.ogg'), node=suit))
            result.append(Parallel(moveTrack, animTrack, suitTrack, damageTrack, soundTrack))
            ...
            damageTrack = Sequence(Wait(3.5), Func(suit.showHpText, -hp, openEnded=0, attackTrack=TRAP_TRACK, attackLevel=3), Func(suit.updateHealthBar, hp))
            ...
            damageTrack = Sequence(Wait(3.5), Func(suit.showHpText, -hp, openEnded=0, attackTrack=TRAP_TRACK, attackLevel=4), Func(suit.updateHealthBar, hp))
            ...
            damageTrack = Sequence(Wait(2.3), Func(suit.showHpText, -hp, openEnded=0, attackTrack=TRAP_TRACK, attackLevel=5), Func(suit.updateHealthBar, hp))
            explosionSound = loader.loadSfx('phase_3.5/audio/sfx/ENC_cogfall_apart_%s.ogg' % random.randint(1, 6))
...
def createSuitReactionToTrain(battle, suit, hp, lure, trapProp):
    ...
    suitTrack = Sequence()
    showDamage = Func(suit.showHpText, -hp, openEnded=0, attackTrack=TRAP_TRACK, attackLevel=6)
