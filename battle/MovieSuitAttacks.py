# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.battle.MovieSuitAttacks
from panda3d.core import Camera, CardMaker, GeomNode, NodePath, Point2, Point3, Point4, Quat, TextNode, Texture, VBase3, VBase4, Vec3, Vec4, headsUp, lookAt
...
from toontown.effects.VisualEffect import *
import string
notify = DirectNotifyGlobal.directNotify.newCategory('MovieSuitAttacks')

def __doDamage(toon, dmg):
...
def doSuitAttack(attack):
    ...
    if name == AUDIT:
        ...
        suitTrack = doSoundImmunity(attack)
    elif name in (TOONS_MISS,
     COGS_HIT,
     POWER_UP,
     POWER_DOWN):
        suitTrack = doHandicap(attack)
    elif name == COG_UP:
        suitTrack = doCogUp(attack)
    elif name == KEY_LOG:
        suitTrack = doKeyLog(attack)
    elif name == DEPRECATE:
        suitTrack = doDeprecate(attack)
    ...
    suit = attack['suit']
    suitTrack = Sequence(suitTrack, Func(suit.loop, 'neutral'))
    ...
    suitTrack = Sequence(suitTrack, toonHprTrack)
...
def __makeCancelledNodePath(text = TTLocalizer.MovieSuitCancelled, times = 3, color = (0.7, 0, 0, 1)):
    tn = TextNode('BattleText')
    tn.setFont(getSuitFont())
    tn.setText('\n'.join([text.upper()] * times))
    tn.setAlign(TextNode.ACenter)
    tn.setTextColor(*color)
    tnpath = NodePath(tn)
    tnpath2 = tnpath.copyTo(tnpath)
    tnpath2.setH(180)
    return tnpath
...
def getSuitTrack(attack, delay = 1e-06, splicedAnims = None, headsUp = True):
    suit = attack['suit']
    battle = attack['battle']
...
def getSuitTauntTrack(attack):
    suit = attack['suit']
    tauntIndex = attack['taunt']
    taunt = getAttackTaunt(attack['name'], tauntIndex)
    return Func(suit.setChatAbsolute, taunt, CFSpeech | CFTimeout)


def getSuitAnimTrack(attack, delay = 0):
    suit = attack['suit']
    return Sequence(Wait(delay), getSuitTauntTrack(attack), ActorInterval(suit, attack['animName']), Func(suit.loop, 'neutral'), Func(suit.clearChat))
...
def getToonTakeDamageTrack(toon, attack, died, dmg, delay, damageAnimNames = None, splicedDamageAnims = None, showDamageExtraTime = 0.01):
    ...
    elif splicedDamageAnims:
        splicedAnims = getSplicedAnimsTrack(splicedDamageAnims, actor=toon)
        toonTrack.append(splicedAnims)
    indicatorTrack = Sequence(Wait(delay + showDamageExtraTime), Func(__doDamage, toon, dmg))
    toonTrack.append(Func(toon.loop, 'neutral'))
    if died:
...
def doCanned(attack):
    ...
    if dmg > 0:
        ...
        throwTrack.append(Func(can.wrtReparentTo, hips))
...
def doSacked(attack):
    ...
    if dmg > 0:
        ...
        sackAppearTrack.append(Func(sack.wrtReparentTo, hips))
...
def doCigarSmoke(attack):
    ...
    suitTrack = getSuitTrack(attack)
    cigarPosPoints = [Point3(-0.13, -0.83, -0.38), VBase3(180.0, 0.0, 0.0)]
...
def doKeyLog(attack):
    ...
    dodged = target['hp'] <= 0
    suitTrack = getSuitTrack(attack)
    toonTrack = getToonTrack(attack, damageDelay=1, damageAnimNames=['conked'], dodgeDelay=1, dodgeAnimNames=[])
    if dodged:
        return Parallel(suitTrack, toonTrack)
    soundTrack = getSoundTrack('SA_buzz_word.ogg', delay=1.5, node=suit)
    wordTrack = Parallel()
    for i in xrange(4):
        startPos = toon.getPos(battle)
        startPos.setZ(startPos.getZ() + toon.getHeight() / 2)
        finalPos = suit.getPos(battle)
        finalPos.setZ(finalPos.getZ() + suit.getHeight() / 2)
        word = __makeCancelledNodePath(random.choice(TTLocalizer.TechbotKeylogWords), 1, (0,
         random.uniform(0.7, 1.0),
         0,
         1))
        word.reparentTo(battle)
        word.setPos(startPos)
        word.hide()
        wordTrack.append(Sequence(Wait(0.8 + i * random.uniform(0.5, 0.7)), Func(word.show), word.posInterval(1, finalPos, blendType='easeInOut'), Func(word.removeNode)))

    return Parallel(suitTrack, wordTrack, toonTrack, soundTrack)


def __makeMonitor(suit, time = 1):
    desk = loader.loadModel('phase_3.5/models/modules/desk_only_wo_phone')
    desk.reparentTo(suit)
    desk.setPos(1.75, 2.5, 0)
    desk.setScale(0.01)
    desk.hide()
    monitor = loader.loadModel('phase_4/models/props/computer_monitor')
    monitor.reparentTo(desk)
    monitor.setPos(-1.75, 1, 2.45)
    monitor.setHpr(270, 0, 0)
    monitor.setScale(5)
    growSequence = Sequence(Func(desk.show), desk.scaleInterval(time, suit.getHeight() / 5.76, desk.getScale(), blendType='easeInOut'))
    return (desk, monitor, growSequence)


def __shrinkMonitor(desk, time = 1):
    return Sequence(desk.scaleInterval(time, 0.01, blendType='easeInOut'), Func(desk.removeNode))


def __makeBrickWall(toon, time = 1):
    wall = loader.loadModel('phase_3/models/misc/box')
    wall.reparentTo(toon)
    wall.setPos(-1.45, 2, 0)
    wall.setColorScale(0.9, 0.9, 0.9, 1)
    wall.setTexture(loader.loadTexture('phase_3/maps/bricks.jpg'), 1)
    wall.hide()
    wallScale = (3, 0.3, 0.01)
    wall.setScale(*wallScale)
    sfx = loader.loadSfx('phase_9/audio/sfx/CHQ_VP_tractor_treads.ogg')
    growSequence = Parallel(SoundInterval(sfx), Func(wall.show), wall.scaleInterval(time, (wallScale[0], wallScale[1], localAvatar.getHeight() + 0.5), wallScale, blendType='easeInOut'))
    return (wall, growSequence)


def __shrinkBrickWall(wall, time = 1):
    scale = wall.getScale()
    return Sequence(wall.scaleInterval(time, (scale[0], scale[1], 0.01), blendType='easeInOut'), Func(wall.removeNode))


def doDeprecate(attack):
    suit = attack['suit']
    battle = attack['battle']
    target = attack['target']
    toon = target['toon']
    dodged = target['hp'] <= 0
    desk, monitor, makeMonitorTrack = __makeMonitor(suit, 2)
    shrinkMonitorTrack = __shrinkMonitor(desk, 2)
    suitTrack = Sequence(makeMonitorTrack, getSuitTrack(attack), shrinkMonitorTrack)
    toonTrack = getToonTrack(attack, damageDelay=3.3, damageAnimNames=['slip-backward'], dodgeDelay=3.3, dodgeAnimNames=[], showMissedExtraTime=1)
    if dodged:
        wall, makeWallTrack = __makeBrickWall(toon, 2)
        shrinkWallTrack = __shrinkBrickWall(wall, 2)
        wallTrack = Sequence(Wait(1), makeWallTrack, Wait(1.5), shrinkWallTrack)
        return Parallel(wallTrack, suitTrack, toonTrack)
    shrinkTrack = Sequence(Wait(3.3), toon.scaleInterval(1, 0.01, 1, blendType='easeInOut'), toon.scaleInterval(1, 1.5, 0.01, blendType='easeInOut'), toon.scaleInterval(1, 1, 1.5, blendType='easeInOut'), Func(toon.setScale, 1))
    shrinkSound = globalBattleSoundCache.getSound('SA_head_shrink_only.ogg')
    growSound = globalBattleSoundCache.getSound('SA_head_grow_back_only.ogg')
    soundTrack = Sequence(Wait(3.3), SoundInterval(shrinkSound, duration=1, node=suit), Wait(1), SoundInterval(growSound, node=suit))
    return Parallel(suitTrack, toonTrack, shrinkTrack, soundTrack)
...
def doDisassemble(attack):
    ...
    shrinkMonitorTrack = __shrinkMonitor(desk, 2)
    suitTrack = Sequence(makeMonitorTrack, getSuitTrack(attack), shrinkMonitorTrack)
    toonTrack = getToonTrack(attack, damageDelay=3.3, damageAnimNames=[], dodgeDelay=3.3, dodgeAnimNames=[], showMissedExtraTime=1)
    if dodged:
        wall, makeWallTrack = __makeBrickWall(toon, 2)
        shrinkWallTrack = __shrinkBrickWall(wall, 2)
        wallTrack = Sequence(Wait(1), makeWallTrack, Wait(1.5), shrinkWallTrack)
    ...
    newGeom = NodePath(geom.node().copySubgraph())
    newGeom.reparentTo(battle)
    newGeom.setPos(geom.getPos(battle))
    newGeom.hide()
    disassembleTrack = Sequence(Wait(3.3), Func(geom.hide), Func(newGeom.show))
    disassembleAllTrack = Parallel()
    parts = {}
    for i, name in enumerate(('head', 'torso', 'legs')):
        part = newGeom.find('**/__Actor_' + name)
        parts[part] = (part.getPos(battle), part.getHpr(battle))
        randomHpr = (random.randint(0, 360), random.randint(0, 360), random.randint(0, 360))
        randomPos = (random.uniform(-3, 3), random.uniform(-3.5, 1), 0.5)
        sfx = globalBattleSoundCache.getSound('SA_powertie_throw.ogg')
        disassembleAllTrack.append(Sequence(Wait((i + 1) * 0.5), Func(sfx.play), Func(part.wrtReparentTo, battle), Parallel(part.posInterval(1, randomPos, blendType='easeInOut'), part.hprInterval(1, randomHpr, blendType='easeInOut'))))

    disassembleTrack.append(disassembleAllTrack)
    reassembleAllTrack = Parallel()
    for part, posInfo in parts.iteritems():
        oldPos, oldHpr = posInfo
        reassembleAllTrack.append(Sequence(Wait((i + 1) * 0.5), Parallel(part.posInterval(1, oldPos, blendType='easeInOut'), part.hprInterval(1, oldHpr, blendType='easeInOut'))))

    disassembleTrack.append(reassembleAllTrack)
    for part in parts:
        disassembleTrack.append(Func(part.removeNode))

    disassembleTrack.append(Func(newGeom.removeNode))
    disassembleTrack.append(Func(geom.show))
    return Parallel(suitTrack, toonTrack, disassembleTrack)
...
def doVoodooMagic(attack):
    ...
    toon = target['toon']
    dodged = target['hp'] <= 0
    parent = suit.getParent()
    toonTrack = getToonTrack(attack, 2.1, ['cringe'], 1.51, ['sidestep'])
    suitTrack = Parallel(Sequence(Func(suit.setTransparency, True), suit.colorScaleInterval(1, (1, 1, 1, 0)), Func(suit.wrtReparentTo, battle), Func(suit.setH, suit.getH(battle) + 180), Func(suit.setPos, toon.getPos(battle)), Func(suit.setY, -10), suit.colorScaleInterval(1, (1, 1, 1, 1)), Wait(3.5), suit.colorScaleInterval(0.5, (1, 1, 1, 0)), Func(suit.reparentTo, parent), Func(suit.setPos, suit.getPos(parent)), Func(suit.setH, suit.getH(parent)), suit.colorScaleInterval(0.5, (1, 1, 1, 1)), Func(suit.setTransparency, False)), getSuitTrack(attack, delay=1.5, headsUp=False))
    if dodged:
        return Parallel(suitTrack, toonTrack)
    bill = globalPropPool.getProp('1dollar')
    billPosPoints = [Point3(-0.01, 0.45, -0.25), VBase3(136.424, -46.434, -129.712)]
    billPropTrack = getPropTrack(bill, suit.getRightHand(), billPosPoints, 1.6, 0.55, scaleUpPoint=Point3(1.41, 1.41, 1.41))
    soundTrack = getSoundTrack('SA_pick_pocket.ogg', delay=1.7, node=suit)
    return Parallel(suitTrack, toonTrack, billPropTrack, soundTrack)
...
def doBash(attack):
    ...
    battle = attack['battle']
    target = attack['target']
    shakeCamera = ShakeCamera(0.5, 1, 60)
    desk, monitor, makeMonitorTrack = __makeMonitor(suit, 2)
    shrinkMonitorTrack = __shrinkMonitor(desk, 2)
    suitTrack = Sequence(makeMonitorTrack, getSuitAnimTrack(attack), shrinkMonitorTrack)
    shakeTrack = Sequence(Wait(3.3), Func(shakeCamera.start), Wait(1.5), Func(shakeCamera.stop))
    toonTracks = Parallel()
    for tgt in target:
        dodged = tgt['hp'] <= 0
        if dodged:
            toonTracks.append(getToonTrack(attack, 0, [], 3, ['jump'], target=tgt))
            continue
        toon = tgt['toon']
        pos = toon.getPos()
        newPos = toon.getPos() + Vec3(0, 0, random.uniform(3.5, 5.0))
        toonTrack = Sequence(Wait(3.3), Parallel(getToonTrack(attack, 0, ['slip-backward'], target=tgt), Sequence(ProjectileInterval(toon, duration=1.4, startPos=pos, endPos=newPos, gravityMult=0.5), ProjectileInterval(toon, duration=1.4, startPos=newPos, endPos=pos, gravityMult=0.5))))
        toonTracks.append(toonTrack)

    return Parallel(suitTrack, shakeTrack, toonTracks)
...
def doDataCorruption(attack):
    ...
    for tgt in target:
        ...
        if dodged:
            wall, makeWallTrack = __makeBrickWall(toon, 2)
            shrinkWallTrack = __shrinkBrickWall(wall, 2)
            wallTrack = Sequence(Wait(1), makeWallTrack, Func(MovieUtil.indicateMissed, toon), Wait(1.5), shrinkWallTrack)
            toonTracks.append(wallTrack)
            continue
        allParts = [toon.getHeadParts(), toon.getTorsoParts(), toon.getLegsParts()]
        animTrack = getToonTrack(attack, 0, ['cringe'], target=tgt)
        partTrack = Sequence()
        for i in xrange(10):
            parts = random.choice(allParts)
            ...
            if rand == 0:
                color = (random.uniform(0.0, 1.0),
                 ...
                 1)
                for part in parts:
                    partTrack.append(Func(part.setColorScale, color))

            elif rand == 1:
                for part in parts:
                    partTrack.append(Func(part.setScale, random.uniform(0.5, 2.0)))

            partTrack.append(Wait(0.3))

        for parts in allParts:
            for part in parts:
                partTrack.append(Func(part.show))
                partTrack.append(Func(part.setColorScale, part.getColorScale()))
                partTrack.append(Func(part.setScale, part.getScale()))

        toonTrack = Sequence(Wait(3.3), Parallel(animTrack, partTrack))
        toonTracks.append(toonTrack)
...
def doElectrostaticEnergy(attack):
    ...
    battle = attack['battle']
    target = attack['target']
    plasma = VisualEffect(battle, texture='phase_4/maps/vfx_plasma.png')
    blast = VisualEffect(battle, texture='phase_4/maps/vfx_blast.png', depth=False)

    def reparentToSuit(plasma):
        plasma.effect.setPos(-0.55, 3.8, 2.7)
        plasma.effect.reparentTo(suit)
        plasma.effect.wrtReparentTo(battle)

    def reparentToPlasma(blast, plasma):
        blast.effect.reparentTo(plasma.effect)
        blast.effect.wrtReparentTo(battle)

    suitTrack = Sequence(getSuitTauntTrack(attack), ActorInterval(suit, 'glower', startFrame=0, endFrame=30), Wait(1), Func(reparentToSuit, plasma), Func(plasma.loop, 0.03), plasma.effect.scaleInterval(0.5, 1, 0, blendType='easeInOut'), Wait(1), ProjectileInterval(plasma.effect, duration=1.5, endPos=(0, 0, 7.5), gravityMult=0.5), Func(reparentToPlasma, blast, plasma), Func(plasma.removeNode), Func(blast.effect.setScale, 1), Func(blast.play, 0.03), Wait(1), ActorInterval(suit, 'glower', startFrame=30), Func(suit.clearChat))
    toonTracks = Parallel()
    for tgt in target:
        dodged = tgt['hp'] <= 0
        toon = tgt['toon']
        if dodged:
            wall, makeWallTrack = __makeBrickWall(toon, 2)
            shrinkWallTrack = __shrinkBrickWall(wall, 2)
            toonTrack = Sequence(Wait(3.3), makeWallTrack, Func(MovieUtil.indicateMissed, toon), Wait(1.5), shrinkWallTrack)
            lightningTarget = wall
        else:
            lightningTarget = toon
            toonTrack = Sequence(Wait(5.3), Func(toon.getGeomNode().setColorScale, (0.64, 0.94, 0.94, 1)), getToonTrack(attack, 0.2, ['cringe'], target=tgt), Func(toon.getGeomNode().clearColorScale))
        fromPos = battle.attachNewNode('FromPos')
        fromPos.setPos(0, 0, 7.5)
        toPos = battle.attachNewNode('ToPos')
        toPos.setPos(lightningTarget.getPos(battle))
        lightning = P2PVisualEffect(fromPos, toPos, texture='phase_4/maps/vfx_lightning.png', scale=1)
        lightningTrack = Sequence(Wait(5.3), Func(lightning.play, 0.03), Wait(1), Func(lightning.removeNode), Func(fromPos.removeNode), Func(toPos.removeNode))
        toonTrack = Parallel(toonTrack, lightningTrack)
        toonTracks.append(toonTrack)

    return Sequence(Parallel(suitTrack, toonTracks), Func(blast.removeNode))
...
def doCloudStorage(attack):
    ...
    toon = target['toon']
    dodged = target['hp'] <= 0
    toonTrack = getToonTrack(attack, 8.4, ['slip-backward'], 3.5, ['sidestep'])
    suitTrack = getSuitTrack(attack)
    cloud = globalPropPool.getProp('stormcloud')
    cloudScale = (6, 6, toon.getHeight() * 3)
    cloudTrack = Sequence(Func(cloud.reparentTo, suit), Func(cloud.setZ, suit.getHeight() / 6), Func(cloud.wrtReparentTo, battle), Func(cloud.setScale, 0), Wait(2.9))
    if dodged:
        cloudTrack.append(Parallel(ProjectileInterval(cloud, duration=1.5, endPos=toon.getPos(battle), gravityMult=0.5), cloud.scaleInterval(1.5, cloudScale, 0, blendType='easeInOut')))
        cloudTrack.append(cloud.scaleInterval(0.5, 0, blendType='easeInOut'))
        cloudTrack.append(Func(MovieUtil.removeProp, cloud))
    else:
        oldScale = toon.getGeomNode().getScale()
        cloudTrack.append(Parallel(ProjectileInterval(cloud, duration=2.5, endPos=toon.getPos(battle), gravityMult=0.5), cloud.scaleInterval(2.5, cloudScale, 0, blendType='easeInOut'), toon.getGeomNode().scaleInterval(2.5, 0.5, oldScale, blendType='easeInOut')))
        cloudTrack.append(Wait(1))
        cloudTrack.append(Parallel(cloud.scaleInterval(2, 0.01, blendType='easeInOut'), toon.getGeomNode().scaleInterval(2, 0.01, blendType='easeInOut')))
        cloudTrack.append(Func(MovieUtil.removeProp, cloud))
        cloudTrack.append(toon.getGeomNode().scaleInterval(2, oldScale, blendType='easeInOut'))
        cloudTrack.append(Func(toon.getGeomNode().setScale, oldScale))
    return Parallel(toonTrack, suitTrack, cloudTrack)


def __makeDisc():
    card = CardMaker('disc')
    card.setFrame(-1, 1, -1, 1)
    disc = NodePath(card.generate())
    disc.setTexture(loader.loadTexture('phase_3/maps/dvd.png'), 1)
    disc.setTransparency(True)
...
def __makeGlitch():
    return None
    card = CardMaker('glitch')
    card.setFrame(-4, 4, -4, 4)
    texture = loader.loadTexture('phase_4/videos/glitch_%s.mp4' % random.randint(1, 2))
    glitch = NodePath(card.generate())
    glitch.setTexture(texture, 1)
    return glitch
...
def doDiskScratch(attack):
    ...
    target = attack['target']
    disc = __makeDisc()
    disc.setHpr(270, 90, 0)
    disc.setPos(0, 0.5, -0.25)
    disc.setScale(0.01)
    suitTrack = Parallel(Sequence(getSuitAnimTrack(attack), Func(disc.removeNode)), Sequence(Func(disc.reparentTo, suit.getLeftHand()), disc.scaleInterval(2.3, 0.5, 0.01, blendType='easeInOut')))
    soundTrack = Sequence(Wait(2.3), SoundInterval(globalBattleSoundCache.getSound('SA_writeoff_pen_only.ogg'), duration=0.9, node=suit), SoundInterval(globalBattleSoundCache.getSound('SA_writeoff_ding_only.ogg'), node=suit))
    toonTracks = Parallel()
    for tgt in target:
        dodged = tgt['hp'] <= 0
        toon = tgt['toon']
        toonTrack = getToonTrack(attack, 2.75, ['cringe'], 2.75, ['duck'], target=tgt)
        if dodged or toon.doId != base.localAvatar.doId:
            toonTracks.append(toonTrack)
            continue
        toonTrack = Parallel(toonTrack)
        glitchTrack = Sequence()
        glitchTrack.append(Wait(2.75))
        for i in xrange(3):
            color = (random.uniform(0.0, 1.0),
             random.uniform(0.0, 1.0),
             random.uniform(0.0, 1.0),
             1)
            glitchTrack.append(Func(render.setColorScale, color))
            glitchTrack.append(Func(aspect2d.setColorScale, color))
            glitchTrack.append(Wait(0.3))

        glitchTrack.append(Func(render.clearColorScale))
        glitchTrack.append(Func(aspect2d.clearColorScale))
        try:
            glitch = __makeGlitch()
            if not glitch:
                raise Exception('No glitch!')
        except:
            toonTrack.append(glitchTrack)
            toonTracks.append(toonTrack)
            continue

        glitchSfx = loader.loadSfx('phase_4/audio/sfx/glitch.ogg')
        glitchTrack.append(Func(glitchSfx.play))
        glitchTrack.append(Func(glitch.reparentTo, render2d))
        glitchTrack.append(Func(aspect2d.hide))
        glitchTrack.append(Wait(0.66))
        glitchTrack.append(Func(glitchSfx.stop))
        glitchTrack.append(Func(glitch.removeNode))
        glitchTrack.append(Func(aspect2d.show))
        toonTrack.append(glitchTrack)
        toonTracks.append(toonTrack)
...
def doReprogram(attack):
    ...
    toon = target['toon']
    dodged = target['hp'] <= 0
    suitTrack = Sequence(Func(suit.headsUp, toon), getSuitTauntTrack(attack), Wait(1), Func(suit.loop, 'walk'), Func(suit.wrtReparentTo, battle), Func(suit.setTransparency, True), Parallel(Sequence(Wait(2), suit.colorScaleInterval(1, (1, 1, 1, 0)), Func(suit.loop, 'neutral'), Func(suit.setPos, suit.getPos(battle)), suit.colorScaleInterval(1, (1, 1, 1, 1)), Func(suit.clearChat)), suit.posInterval(2.98, toon.getPos(battle))), Func(suit.setTransparency, False))
    toonTrack = getToonTrack(attack, 3.5, ['cringe'], 2.5, ['sidestep'])
    if not dodged:
        toonTrack = Sequence(Parallel(toonTrack, Sequence(Wait(3), toon.hprInterval(0.5, (toon.getH() + 360, toon.getP(), toon.getR())), Func(toon.getGeomNode().setColorScale, (0,
         random.uniform(0.5, 1.0),
         0,
         1)))), Func(toon.getGeomNode().setColorScale, toon.getColorScale()))
    return Parallel(suitTrack, toonTrack)


def doSoundImmunity(attack):
    suit = attack['suit']
    battle = attack['battle']
    phone = globalPropPool.getProp('phone')
    receiver = globalPropPool.getProp('receiver')
    suitTrack = ActorInterval(suit, 'phone')
    phonePosPoints = [Point3(0.23, 0.17, -0.11), VBase3(5.939, 2.763, -177.591)]
    receiverPosPoints = [Point3(0.23, 0.17, -0.11), VBase3(5.939, 2.763, -177.591)]
    propTrack = Sequence(Wait(0.3), Func(__showProp, phone, suit.getLeftHand(), phonePosPoints[0], phonePosPoints[1]), Func(__showProp, receiver, suit.getLeftHand(), receiverPosPoints[0], receiverPosPoints[1]), LerpScaleInterval(phone, 0.5, MovieUtil.PNT3_ONE, MovieUtil.PNT3_NEARZERO), Wait(0.74), Func(receiver.wrtReparentTo, suit.getRightHand()), LerpPosHprInterval(receiver, 0.0001, Point3(-0.45, 0.48, -0.62), VBase3(-87.47, -18.21, 7.82)), Wait(3.14), Func(receiver.wrtReparentTo, phone), Wait(0.62), LerpScaleInterval(phone, 0.5, MovieUtil.PNT3_NEARZERO), Func(MovieUtil.removeProps, [receiver, phone]))
    soundTrack = getSoundTrack('SA_hangup.ogg', delay=1.3, node=suit)
...
def __makeGear():
    gear = loader.loadModel('phase_9/models/char/gearProp')
    gear.setScale(0.1)
    return gear
...
def doCogUp(attack):
    ...
    if len(suits) == 1:
        callback = Sequence(Func(suit.updateHealthBar, 0), Func(suit.showHpText, suit.getMaxHP() / 2))
        return doHandicap(attack, callback)
    geom = suit.getGeomNode()
    origHpr = getHeadsUpHpr(Vec3(0, 0, 0), suit.getPos(battle))
    suitTrack = Sequence(Func(suit.headsUp, battle), Func(suit.loop, 'walk'), Func(suit.wrtReparentTo, battle), Func(suit.updateHealthBar, 0), Func(suit.showHpText, suit.getMaxHP() / 2), suit.posInterval(2, (0, 0, 0), blendType='easeInOut'), Func(suit.loop, 'neutral'))
    gears = [ __makeGear() for i in xrange(3) ]
    suitTracks = Sequence()
    for healedSuit in suits:
        if healedSuit == suit:
            continue
        gearTracks = Parallel()
        targetPos = healedSuit.getMeterJoint().getPos(battle)
        for i, gear in enumerate(gears):
            gearTrack = Sequence(Wait(0.2 * i), Func(gear.show), Func(gear.setPos, 0, 0, 0), Func(gear.reparentTo, suit.getMeterJoint()), Func(gear.wrtReparentTo, battle), gear.posInterval(0.5, targetPos, blendType='easeInOut'), Func(gear.hide))
            gearTracks.append(gearTrack)

        sfx = random.choice(healedSuit.getDialogueArray())
        healedSuitTrack = Sequence(Wait(0.5), Parallel(Sequence(healedSuit.scaleInterval(0.5, 1.2, blendType='easeInOut'), healedSuit.scaleInterval(0.5, 1, blendType='easeInOut')), Sequence(Wait(0.3), Func(healedSuit.updateHealthBar, 0), Func(healedSuit.showHpText, healedSuit.getMaxHP() / 2)), Sequence(healedSuit.getGeomNode().colorScaleInterval(0.5, (0.6, 0.6, 0.6, 1), blendType='easeInOut'), healedSuit.getGeomNode().colorScaleInterval(0.5, (1, 1, 1, 1), blendType='easeInOut')), Func(sfx.play)))
        healTrack = Sequence(Wait(0.5), Func(suit.loop, 'walk'), suit.hprInterval(1.5, getHeadsUpHpr(Vec3(0, 0, 0), healedSuit.getPos(battle)), blendType='easeInOut'), Func(suit.loop, 'neutral'), Parallel(gearTracks, healedSuitTrack))
        suitTracks.append(healTrack)

    suitTrack.append(suitTracks)
    suitTrack.append(Wait(0.5))
    suitTrack.append(Func(suit.loop, 'walk'))
    suitTrack.append(suit.hprInterval(1, origHpr, blendType='easeInOut'))
    suitTrack.append(suit.posInterval(2, suit.getPos(battle), blendType='easeInOut'))
    suitTrack.append(Func(suit.setHpr, suit.getHpr(battle)))
    for gear in gears:
        suitTrack.append(Func(gear.removeNode))

    return suitTrack


def doHandicap(attack, callback = None):
    if not callback:
        callback = Sequence()
    name = attack['name']
    if name in ('ToonsMiss', 'PowerDown'):
        animName = 'victory'
    else:
        animName = 'effort'
    if name in ('PowerUp', 'CogUp'):
        color = (0, 1, 0, 1)
        sound = 'SA_powerup.ogg'
    elif name in ('ToonsMiss', 'CogsHit'):
        color = (1, 0, 0, 1)
        sound = 'target_Lose.ogg'
    elif name == 'PowerDown':
        color = (1, 0.65, 0, 1)
        sound = 'SA_powerdown.ogg'
    suit = attack['suit']
    geom = suit.getGeomNode()
    if animName == 'effort':
        duration = suit.getDuration('effort', fromFrame=50)
        suitTrack = ActorInterval(suit, 'effort', startFrame=50)
    else:
        duration = suit.getDuration(animName)
        suitTrack = ActorInterval(suit, animName)
    suitTrack = Parallel(suitTrack, Sequence(geom.colorScaleInterval(duration / 2, color), callback, geom.colorScaleInterval(duration / 2, (1, 1, 1, 1))))
    soundTrack = getSoundTrack(sound, node=suit)
