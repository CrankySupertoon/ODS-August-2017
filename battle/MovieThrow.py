# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.battle.MovieThrow
...
def __throwPie(throw, delay, hitCount):
    ...
    pieShow = Func(pie.reparentTo, hand)
    ...
    pieTrack = Sequence(Wait(delay), pieShow, pieAnim, pieScale, Func(battle.movie.needRestoreRenderProp, pie), Wait(tPieLeavesHand - 1.0), piePreflight)
    ...
    if hitSuit:
        ...
        pieHide = Func(MovieUtil.removeProp, pie)
        ...
        pieTrack.append(Func(battle.movie.clearRenderProp, pie))
    ...
    if hitSuit:
        ...
        showDamage = Func(suit.showHpText, -hp, openEnded=0, attackTrack=THROW_TRACK, attackLevel=level)
        ...
        if kbbonus > 0:
            ...
            animTrack = Sequence()
            animTrack.append(ActorInterval(suit, 'pie-small', duration=0.2))
            if suitType == 'a':
            ...
            sival = Parallel(ActorInterval(suit, 'pie-small'), MovieUtil.createSuitStunInterval(suit, 0.3, 1.3))
        else:
            sival = ActorInterval(suit, 'pie-small')
...
def __createWeddingCakeFlight(throw, groupHitDict, pie, pies):
    ...
    for i in xrange(numTargets):
        ...
        if hitSuit:
            ...
            singlePieTrack.append(Func(battle.movie.clearRenderProp, pie))
...
def __throwGroupPie(throw, delay, groupHitDict):
    ...
    pieShow = Func(pie.reparentTo, hand)
    pieAnim = Func(__animProp, pies, pieName, pieType)
    pieScale = LerpScaleInterval(pie, 1.0, pie.getScale() * 1.5, startScale=MovieUtil.PNT3_NEARZERO)
