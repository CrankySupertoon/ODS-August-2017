# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.battle.MovieUtil
from panda3d.core import ColorBlendAttrib, GeomNode, Point3, Vec3, lookAt
...
def showProps(props, hands, pos = None, hpr = None, scale = None):
    for i, prop in enumerate(props):
        showProp(prop, hands[i], pos, hpr, scale)
...
def virtualize(deathsuit):
    for part in deathsuit.getGeomNode().findAllMatches('*'):
        if part.getName() in ('joint_attachMeter', 'joint_nameTag', 'def_nameTag'):
            continue
        part.setColorScale(1.0, 0.0, 0.0, 1.0)
...
def hideDeathSuit(deathSuit):
    if not deathSuit:
        return
    for node in ('legs', 'hands', 'torso', 'arms', 'Lhand', 'Rhand'):
        node = deathSuit.find('**/' + node)
...
def createSuitDeathTrack(suit, toon, battle, npcs = []):
    ...
    suitTrack.append(Parallel(ActorInterval(deathSuit, 'lose', duration=SUIT_LOSE_DURATION), Sequence(Wait(SUIT_LOSE_TORSO_DURATION), Func(hideDeathSuit, deathSuit))))
...
def getToonTeleportOutInterval(toon):
    ...
    hole.clearBin()
    hole.setBin('fixed', 40)
    delay = T_HOLE_LEAVES_HAND
    dur = T_TELEPORT_ANIM
    holeTrack = Sequence()
    holeTrack.append(Func(hole.reparentTo, toon.getRightHand()))
    ...
    holeAnimTrack.append(Func(hole.detachNode))
    runTrack = Sequence(ActorInterval(toon, 'teleport', duration=dur), Func(toon.getGeomNode().hide), Wait(T_HOLE_CLOSES), Func(toon.detachNode))
...
def createSuitStunInterval(suit, before, after):
    ...
    return Sequence(Wait(before), Func(stars.reparentTo, head), Func(stars.setZ, max(0.0, p2[2] - 1.0)), Func(stars.loop, 'stun'), Wait(after), Func(stars.delete))
