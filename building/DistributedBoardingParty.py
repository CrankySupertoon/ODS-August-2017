# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.building.DistributedBoardingParty
...
class DistributedBoardingParty(DistributedObject.DistributedObject, BoardingPartyBase.BoardingPartyBase):
    ...
    def postGroupInfo(self, leaderId, memberList, inviteeList, kickedList, practice):
        ...
        if isMyGroup:
            ...
            self.groupPanel.setPractice(practice)
    ...
    def postInviteNotQualify(self, avId, reason):
        ...
        if avId == localAvatar.doId:
            ...
            if reason == BoardingPartyBase.BOARDCODE_GROUPS_TOO_LARGE:
    ...
    def requestPractice(self, practice):
        self.sendUpdate('requestPractice', [practice])

    def postPractice(self, practice):
        if self.groupPanel:
            self.groupPanel.setPractice(practice)

    def setPracticeEnabled(self, practiceEnabled):
        self.practiceEnabled = practiceEnabled

    def getPracticeEnabled(self):
        return self.practiceEnabled
    ...
    def handleEnterElevator(self, elevator):
        if not self.canEnterElevator or self.goToPreShowTrack is not None and self.goToPreShowTrack.isPlaying() or self.goToShowTrack is not None and self.goToShowTrack.isPlaying():
            return
        else:
