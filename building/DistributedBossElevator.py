# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.building.DistributedBossElevator
...
class DistributedBossElevator(DistributedElevatorExt.DistributedElevatorExt):
    ...
    def generate(self):
        ...
        self.accept('requestPractice', self.d_requestPractice)
    ...
    def d_requestPractice(self, practice):
        self.sendUpdate('requestPractice', [practice])

    def toonBoarded(self, toon):
        if toon == base.localAvatar and not toon.readyForPromotion(self.type - 1):
            base.localAvatar.setSystemMessage(0, TTLocalizer.BoardNoMeritsMessage)
        elevator = self.getPlaceElevator()
        if not elevator or not elevator.practiceButton:
            return
        if len(self.boardedAvIds) == 1:
            elevator.practiceButton.show()
        else:
            elevator.practiceButton.hide()
