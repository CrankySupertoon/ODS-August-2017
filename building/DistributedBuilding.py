# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.building.DistributedBuilding
from panda3d.core import DecalEffect, Lens, NodePath, Point3, TextNode, VBase4, Vec3
...
class DistributedBuilding(DistributedObject.DistributedObject):
    ...
    def getNodePaths(self):
        nodePath = []
        npc = self.cr.playGame.hood.loader.geom.findAllMatches('**/?b' + str(self.block) + ':*_DNARoot;+s')
    ...
    def setupSuitBuilding(self, nodePath):
        ...
        suitNP = assetStorage.findNode('suit_landmark_' + chr(self.track))
        ...
        buildingTitle += TTLocalizer.CogsIncExt.get(chr(self.track), TTLocalizer.CogsIncExtDefault)
        ...
        if not frontNP.isEmpty():
            backgroundNP.wrtReparentTo(frontNP)
            frontNP.node().setEffect(DecalEffect.make())
        signTextNodePath.setDepthOffset(5)
        ...
        if chr(self.track) == 't':
            suitBuildingNP.find('**/*_front').setScale(1, 1, 1.01)
            suitBuildingNP.find('**/*_front').setPos(0, 0, -0.14)
            suitBuildingNP.find('**/suit_sign*').setPos(12.8, -2.8, 11.5)
            suitBuildingNP.find('**/*door_origin').setPos(14.3, -4, 0)
    ...
    def setupCogdo(self, nodePath):
        dnaStore = self.cr.playGame.dnaStore
        level = int(self.difficulty / 2) + 1
        suitNP = assetStorage.findNode(FO_DICT[chr(self.track)])
        if not suitNP:
    ...
    def walkOutCameraTrack(self):
        track = Sequence(Func(camera.reparentTo, render), Func(camera.setPosHpr, self.elevatorNodePath, 0, -32.5, 9.4, 0, 348, 0), Func(base.camLens.setMinFov, 52.0 / (4.0 / 3.0)), Wait(VICTORY_RUN_TIME), Func(camera.setPosHpr, self.elevatorNodePath, 0, -32.5, 17, 0, 347, 0), Func(base.camLens.setMinFov, 75.0 / (4.0 / 3.0)), Wait(TO_TOON_BLDG_TIME), Func(base.camLens.setMinFov, 52.0 / (4.0 / 3.0)))
        return track
