# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.building.DistributedDoor
...
class DistributedDoor(DistributedObject.DistributedObject, DelayDeletable):
    ...
    def setupNametag(self):
        if not self.wantsNametag():
            ...
            if self.nametag == None:
                ...
                bldgType = self.cr.playGame.dnaStore.getBlockBuildingType(self.block)
                if bldgType == 'toptoons':
                    self.nametag.setColorCode(NametagGroup.CCSpecialBuilding)
                else:
    ...
    def doPostAnnounceGenerate(self):
        if self.doneWithGeneration or not self.getBuilding():
            return
        self.doneWithGeneration = True
        self.bHasFlat = self.doorType in [DoorTypes.INT_STANDARD, DoorTypes.INT_HQ] and not self.findDoorNode('door*flat', True).isEmpty()
        self.hideDoorParts()
        self.setTriggerName()
    ...
    def isDoorHit(self):
        ...
        if not door:
            ...
            facingForward = -100.0 < base.localAvatar.getH(door) < 100.0
            return yToTest and facingForward
    ...
    def destroyRejectDialog(self):
        if self.rejectDialog:
    ...
    def __faRejectEnter(self, message):
        ...
        self.acceptOnce('clientCleanup', self.__handleClientCleanup)
    ...
    def getDoorNodePath(self):
        if self.doorType == DoorTypes.INT_STANDARD:
            return render.find('**/door_origin')
        if self.doorType == DoorTypes.EXT_STANDARD:
            building = self.getBuilding()
            if building and not building.isEmpty():
                return building.find('**/*door_origin')
        elif self.doorType in self.specialDoorTypes:
            building = self.getBuilding()
            if building and not building.isEmpty():
                return building.find('**/door_origin_' + str(self.doorIndex))
        else:
            if self.doorType == DoorTypes.INT_HQ:
                return render.find('**/door_origin_' + str(self.doorIndex))
            self.notify.error('No such door type as ' + str(self.doorType))
    ...
    def enterClosing(self, ts):
        ...
        if self.rightSwing:
            ...
            h = -100
        self.finishDoorTrack()
        self.doorTrack = Sequence(LerpHprInterval(nodePath=rightDoor, duration=1.0, hpr=VBase3(0, 0, 0), startHpr=VBase3(h, 0, 0), other=otherNP, blendType='easeInOut'), Func(doorFrameHoleRight.hide), Func(self.doPostAnnounceGenerate), Func(self.hideIfHasFlat, rightDoor), SoundInterval(self.closeSfx, node=rightDoor), name=trackName)
    ...
    def exitDoorEnterClosing(self, ts):
        ...
        if not leftDoor.isEmpty():
            ...
            self.finishDoorExitTrack()
            self.doorExitTrack = Sequence(LerpHprInterval(nodePath=leftDoor, duration=1.0, hpr=VBase3(0, 0, 0), startHpr=VBase3(h, 0, 0), other=otherNP, blendType='easeInOut'), Func(doorFrameHoleLeft.hide), Func(self.doPostAnnounceGenerate), Func(self.hideIfHasFlat, leftDoor), SoundInterval(self.closeSfx, node=leftDoor), name=trackName)
