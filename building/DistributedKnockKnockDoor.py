# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.building.DistributedKnockKnockDoor
...
class DistributedKnockKnockDoor(DistributedAnimatedProp.DistributedAnimatedProp):
    ...
    def knockKnockTrack(self, avatar, duration):
        if avatar is None:
            ...
            self.knockSfx = loader.loadSfx('phase_5/audio/sfx/GUI_knock_%s.ogg' % random.randint(1, 4))
    ...
    def cleanupTrack(self):
        ...
        if avatar:
            avatar.clearChat()
        if hasattr(self, 'nametag') and self.nametag:
