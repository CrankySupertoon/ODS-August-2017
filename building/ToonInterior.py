# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.building.ToonInterior
...
import RandomBuilding
import random
InteriorTypes = {'toon_interior': [-0.122, 10.285, 1.1],
 'toon_interior_2': [-0.208, 12.405, 0.0],
 'toon_interior_L': [7.3, 8.4, 270.0],
 'toon_interior_T': [-2.9, 5.171, 2.4]}
...
class ToonInterior(Place.Place):
    ...
    def __init__(self, loader, parentFSMState, doneEvent):
        ...
        self.fsm = ClassicFSM.ClassicFSM('ToonInterior', [State.State('start', self.enterStart, self.exitStart, ['doorIn', 'teleportIn', 'tutorial']),
         ...
         State.State('stickerBook', self.enterStickerBook, self.exitStickerBook, ['walk',
          ...
          'pet',
          'NPCFA']),
         State.State('NPCFA', self.enterNPCFA, self.exitNPCFA, ['NPCFAReject',
          'HFA',
          'teleportOut',
          'doorOut']),
    ...
    def enterTeleportIn(self, requestStatus):
        ...
        elif ZoneUtil.isHouse(self.zoneId):
            base.localAvatar.setPosHpr(-12.2, -22.2, ToontownGlobals.FloorOffset, 0, 0, 0)
        else:
            randomGen = random.Random()
            randomGen.seed(self.getZoneId())
            modelType = RandomBuilding.randomDNAItem(randomGen, 'TI_room', assetStorage.findNode)
            modelType = str(modelType.getName().split('.egg')[0])
