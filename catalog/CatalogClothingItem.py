# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.catalog.CatalogClothingItem
...
ClothingTypes = {101: (ABoysShirt, 'bss1', 40),
 ...
 1820: (AShirt, 'ln_404', 5000),
 1821: (AShirt, 'ln_bke', 5000),
 1822: (AShirt, 'ln_dfn', 5000),
 1823: (AShirt, 'ln_key', 5000),
 1824: (AShirt, 'ln_mup', 5000),
 1825: (AShirt, 'ln_ods', 5000),
 1826: (AShirt, 'ln_red', 5000),
 1827: (AShirt, 'ln_tux', 5000),
 1828: (AShirt, 'ln_twi', 5000),
 1829: (ABoysShorts, 'ln_bs404', 5000),
 1830: (ABoysShorts, 'ln_bsdfn', 5000),
 1831: (ABoysShorts, 'ln_bstwi', 5000),
 1832: (AGirlsShorts, 'ln_gs404', 5000),
 1833: (AGirlsSkirt, 'ln_gs404b', 5000),
 1834: (AGirlsShorts, 'ln_gsdfn', 5000),
 1835: (AGirlsShorts, 'ln_gstwi', 5000),
 1836: (AShirt, 'ln_arg', 5000)}
...
class CatalogClothingItem(CatalogItem.CatalogItem):
    ...
    def notOfferedTo(self, avatar):
        ...
        if article in (AShirt, AShorts):
            return False
        forBoys = article in (ABoysShirt, ABoysShorts)
        if avatar.getStyle().getGender() == 'm':
            return not forBoys
        return forBoys

    def forBoysOnly(self):
        return ClothingTypes[self.clothingType][CTArticle] in (ABoysShirt, ABoysShorts)

    def forGirlsOnly(self):
        return ClothingTypes[self.clothingType][CTArticle] in (AGirlsShirt, AGirlsSkirt, AGirlsShorts)
    ...
    def reachedPurchaseLimit(self, avatar):
        ...
        dna = avatar.getStyle()
        if self.isShirt():
            style = ToonDNA.ShirtStyles[str]
            topTex, sleeveTex, colors = ToonDNA.ShirtStyles[str]
            topTexColor, sleeveTexColor = colors[self.colorIndex]
            topTexColor = ToonDNA.migrateColor(topTexColor, True)
            sleeveTexColor = ToonDNA.migrateColor(sleeveTexColor, True)
            if dna.topTex == topTex and dna.topTexColor == topTexColor and dna.sleeveTex == sleeveTex and dna.sleeveTexColor == sleeveTexColor:
                return True
            if (topTex,
             topTexColor,
             sleeveTex,
             sleeveTexColor) in avatar.clothesTopsList:
                return True
        else:
            style = ToonDNA.BottomStyles[str]
            botTex, botTexColor = style[0], style[1][self.colorIndex]
            botTexColor = ToonDNA.migrateColor(botTexColor, True)
            if dna.botTex == botTex and dna.botTexColor == botTexColor:
                return True
            if (botTex, botTexColor) in avatar.clothesBottomsList:
                return True
    ...
    def recordPurchase(self, avatar, optional):
        ...
        dna = avatar.getStyle()
        if self.isShirt():
            avatar.addToClothesTopsList((dna.topTex,
             dna.topTexColor,
             dna.sleeveTex,
             dna.sleeveTexColor))
            dna.topTex, dna.sleeveTex, colors = ToonDNA.ShirtStyles[str]
            dna.topTexColor, dna.sleeveTexColor = colors[self.colorIndex]
        else:
            avatar.addToClothesBottomsList((dna.botTex, dna.botTexColor))
            style = ToonDNA.BottomStyles[str]
            dna.botTex, dna.botTexColor = style[0], style[1][self.colorIndex]
        if dna.getGender() == 'f':
            try:
    ...
    def getPicture(self, avatar):
        ...
        dna = avatar.style.clone()
        str = ClothingTypes[self.clothingType][CTString]
        if self.isShirt():
            dna.topTex, dna.sleeveTex, colors = ToonDNA.ShirtStyles[str]
            dna.topTexColor, dna.sleeveTexColor = colors[self.colorIndex]
            dna.topTexColor = ToonDNA.migrateColor(dna.topTexColor, True)
            dna.sleeveTexColor = ToonDNA.migrateColor(dna.sleeveTexColor, True)
            pieceNames = ('**/torso-top', '**/sleeves')
        else:
            style = ToonDNA.BottomStyles[str]
            dna.botTex, dna.botTexColor = style[0], style[1][self.colorIndex]
            dna.botTexColor = ToonDNA.migrateColor(dna.botTexColor, True)
    ...
    def getFilename(self):
        str = ClothingTypes[self.clothingType][CTString]
        if self.isShirt():
            return ToonDNA.Shirts[ToonDNA.ShirtStyles[str][0]]
        botTex = ToonDNA.BottomStyles[str][0]
        article = ClothingTypes[self.clothingType][CTArticle]
    ...
    def getColor(self):
        str = ClothingTypes[self.clothingType][CTString]
        if self.isShirt():
            return ToonDNA.ClothesColors[ToonDNA.ShirtStyles[str][2][self.colorIndex][0]]
        else:
            return ToonDNA.ClothesColors[ToonDNA.BottomStyles[str][1][self.colorIndex]]
