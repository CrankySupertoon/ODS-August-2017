# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.catalog.CatalogFurnitureItem
from panda3d.core import Datagram, Filename, Light
...
TvToPosScale = {1530: ((-1.15, -0.5, 1.1), (2.5, 1.7, 1.4)),
 1531: ((-2.3, -0.2, 2.522), (5, 3.75, 3.187)),
 1532: ((-7, -0.2, 2.8), (15, 10, 7.8))}
ChairToPosHpr = {100: ((0, -3.9, 0.88),
       (180, 0, 0),
       (0, -4.9, 0),
       -3.0),
 105: ((0, -3.9, 0.88),
...
class CatalogFurnitureItem(CatalogAtticItem.CatalogAtticItem):
    ...
    def needsOwner(self):
        return self.getFlags() & (FLTrunk | FLCloset | FLBank | FLCrate) != 0
    ...
    def isGift(self):
        if self.getEmblemPrices():
            return 0
        elif self.getFlags() & (FLCloset | FLBank):
            return 0
        else:
            return 1
    ...
    def recordPurchase(self, avatar, optional):
        ...
        if self.furnitureType in TvToPosScale:
            messenger.send('statChange', [avatar, ToontownGlobals.STAT_TV, 1])
        if retcode >= 0:
            if self.getFlags() & FLCloset:
    ...
    def loadModel(self):
        ...
        for element in (('**/*shadow*', 0.08), ('**/*Shadow*', 0.06), ('**/*pPlane*', 0.003)):
            name, z = element
            for node in model.findAllMatches(name):
                node.setZ(z)
