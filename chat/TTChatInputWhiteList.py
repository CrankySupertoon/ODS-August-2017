# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.chat.TTChatInputWhiteList
...
class TTChatInputWhiteList(ChatInputWhiteListFrame):
    ...
    def typeCallback(self, extraArgs):
        ...
        if localAvatar.chatMgr.chatInputNormal.isActive():
            return
    ...
    def sendChatByMode(self, text):
        if self.trueFriendChat:
            for friend in base.localAvatar.trueFriends:
                self.sendWhisperByFriend(friend, text)

        else:
            ChatInputWhiteListFrame.sendChatByMode(self, text)

    def sendWhisperByFriend(self, avatarId, text):
        av = base.cr.doId2do.get(avatarId)
        if av and av.isUnderstandable():
