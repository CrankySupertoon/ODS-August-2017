# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.cogdominium.CogdoFlyingLocalPlayer
...
class CogdoFlyingLocalPlayer(CogdoFlyingPlayer):
    ...
    def __init__(self, toon, game, level, guiMgr):
        ...
        self.defaultTransitions = {'Inactive': ['FreeFly', 'Running'],
         ...
         'WaitingForWin': ['Inactive', 'Win', 'FreeFly'],
         'Win': ['Inactive']}
        self.game = game
        self._level = level
        self._guiMgr = guiMgr
    ...
    def stashBodyCollisions(self):
        for av in base.cr.doId2do.values():
            if hasattr(av, 'stashBodyCollisions'):
                av.stashBodyCollisions()

    def unstashBodyCollisions(self):
        for av in base.cr.doId2do.values():
            if hasattr(av, 'unstashBodyCollisions'):
                av.unstashBodyCollisions()
    ...
    def updateToonFlyingState(self, dt):
        ...
        if self.hasPickedUpFirstPropeller:
    ...
    def pressedControlWhileRunning(self):
        if self.isFuelLeft() and self.state == 'Running':
            ...
            self.acceptOnce(base.getKey('fire'), self.pressedControlWhileRunning)
