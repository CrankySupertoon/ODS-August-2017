# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.coghq.DistributedBanquetTable
from panda3d.direct import SmoothMover
from panda3d.core import BitMask32, CollideMask, CollisionHandler, CollisionHandlerQueue, CollisionNode, CollisionSegment, NodePath, Point3, TextNode, VBase4, Vec3, deg2Rad, lookAt
import math
import random
...
class DistributedBanquetTable(DistributedObject.DistributedObject, FSM.FSM, BanquetTableBase.BanquetTableBase):
    ...
    def destroyDiners(self):
        for serviceLoc in self.serviceLocs.values():
            serviceLoc.removeNode()

        for sitLoc in self.sitLocators.values():
            sitLoc.removeNode()

        for statusInd in self.dinerStatusIndicators.values():
            statusInd.delete()
    ...
    def setDinersEnabled(self, enabled):
        if self.diners and not enabled:
            self.destroyDiners()
            return
        if enabled and not self.diners:
    ...
    def createDiner(self, i):
        ...
        diner.destroyNametag3d()
        diner.deleteShadowPlacer()
    ...
    def changeDinerToDead(self, chairIndex):
        ...
        ival = Sequence(ActorInterval(diner, 'sit-lose'), Func(deathSuit.setHpr, diner.getHpr()), Func(diner.hide), Func(deathSuit.reparentTo, self.chairLocators[chairIndex]), ActorInterval(deathSuit, 'lose', duration=MovieUtil.SUIT_LOSE_REVIVE_DURATION), Func(removeDeathSuit, diner, deathSuit, name='remove-death-suit-%d-%d' % (chairIndex, self.index)), Func(diner.stash))
        spinningSound = loader.loadSfx('phase_3.5/audio/sfx/Cog_Death.ogg')
        deathSound = loader.loadSfx('phase_3.5/audio/sfx/ENC_cogfall_apart_%s.ogg' % random.randint(1, 6))
    ...
    def __enableControlInterface(self):
        ...
        self.upKey, self.downKey, self.controlKey = base.getKey('up'), base.getKey('down'), base.getKey('fire')
        self.accept('escape', self.__exitPitcher)
        self.accept(self.controlKey, self.__controlPressed)
        self.accept(self.controlKey + '-up', self.__controlReleased)
        self.accept('InputState-forward', self.__upArrow)
        self.accept('InputState-reverse', self.__downArrow)
        self.accept('InputState-turnLeft', self.__leftArrow)
        self.accept('InputState-turnRight', self.__rightArrow)
        self.accept(self.upKey, self.__upArrowKeyPressed)
        self.accept(self.downKey, self.__downArrowKeyPressed)
    ...
    def __disableControlInterface(self):
        ...
        self.ignore('escape')
        self.ignore(self.controlKey)
        self.ignore(self.controlKey + '-up')
        self.ignore('InputState-forward')
        self.ignore('InputState-reverse')
        self.ignore('InputState-turnLeft')
        self.ignore('InputState-turnRight')
        self.ignore(self.upKey)
        self.ignore(self.downKey)
        self.arrowVert = 0
        self.arrowHorz = 0
        taskMgr.remove(self.watchControlsName)
        taskMgr.remove(self.waterPowerTaskName)
