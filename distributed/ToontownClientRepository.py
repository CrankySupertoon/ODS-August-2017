# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.distributed.ToontownClientRepository
from panda3d.core import Connection, Datagram, DatagramIterator, HTTPClient, ModelPool, Texture, TexturePool
import types
import time
...
from toontown.toontowngui.SleepScreen import SleepScreen
from toontown.toontowngui import TTDialog, GUIUtils
from toontown.toon import LocalToon
from toontown.toon import ToonDNA
from toontown.distributed import ToontownDistrictStats
from toontown.parties import ToontownTimeManager
from toontown.toon import Toon, DistributedToon
from toontown.launcher.LauncherGUI import LauncherGUI
...
class ToontownClientRepository(OTPClientRepository.OTPClientRepository):
    ...
    def __init__(self, serverVersion):
        ...
        self.transparentToons = False
        self.acceptedBackground = None
        self.rejectedBackground = None
        self.noConnectionBackground = None
        self.applauseSound = None
        self.rejectedSound = None
        self.accept('fellAsleep', self.__fellAsleep)
        self.gameServer = config.GetString('gameserver', '127.0.0.1')
        return

    def createLoadingScreen(self):
        base.playMusic(base.getMainTheme(), looping=True)
        loader.beginBulkLoad('init', TTLocalizer.WelcomeToToontown % random.choice(TTLocalizer.WelcomeToToontownWhere), 2, TTLocalizer.TIP_NONE, 0)
        messenger.send('setConnectingText', [TTLocalizer.LoaderLabel])

    def startConnecting(self):
        base.createServerList()
        self.createLoadingScreen()
        self.loginFSM.request('connect', [base.serverList])

    def startLoading(self):
        self.createLoadingScreen()
        self.loginFSM.request('waitForGameList')

    def enterLauncher(self):
        if self.launcher:
            return
        loader.endBulkLoad('init')
        self.launcher = LauncherGUI()
        self.launcher.load()
        self.launcher.enter()
    ...
    def enterConnect(self, serverList = []):
        ...
        if config.GetBool('server-force-ssl', False):
            try:
                if dist == 'dev':
                    serverpem = open('dependencies/ssl/ods_dev.crt', 'r').read()
                else:
                    import ODSGlobals
                    serverpem = ODSGlobals.getcassl(dist)
            except:
                serverpem = None
    ...
    def getApprovedMusic(self):
        return loader.loadMusic('phase_4/audio/bgm/trolley_song.ogg')

    def getRejectedMusic(self):
        return loader.loadMusic('phase_12/audio/bgm/Bossbot_Factory_v2.ogg')

    def getNoConnectionMusic(self):
        return loader.loadMusic('phase_3.5/audio/bgm/thunder_rain_sfx.ogg')

    def handleNameApproved(self, avatarChoice):
        if self.acceptedBackground:
            return
        else:
            if not self.applauseSound:
                self.applauseSound = loader.loadSfx('phase_6/audio/sfx/KART_Applause_%s.ogg' % random.randint(1, 2))
            base.playSfx(self.applauseSound, volume=0.7)
            base.playMusic(self.getApprovedMusic(), looping=True, volume=0.9)
            self.acceptedBackground = GUIUtils.loadRandomGradient(render2d, (-1, 1, -1, 1), 0.1, 0.85, 0.4)
            self.acceptedScroll = GUIUtils.loadTextureModel(loader.loadTexture('phase_3/maps/scroll.png'), transparency=True)
            self.acceptedScroll.reparentTo(aspect2d)
            self.acceptedScroll.setScale(1.1, 1, 0.9)
            self.acceptedBanner = DirectLabel(relief=None, text=TTLocalizer.CRNameCongratulations, text_scale=0.13, text_fg=(0.6, 0.1, 0.1, 1), text_font=getMinnieFont(), pos=(0, 0, 0.485))
            self.acceptedText = DirectLabel(relief=None, text=TTLocalizer.CRNameAccepted % avatarChoice.approvedName, text_scale=0.1, pos=(0, 0, 0.25))
            self.acceptedButton = DirectButton(relief=None, image=Preloaded['okButton'], text=TTLocalizer.lOK, scale=1.35, text_scale=0.05, text_pos=(0.0, -0.1), pos=(0, 0, -0.425), command=self.__handleCongrats, extraArgs=[avatarChoice])
            base.transitions.noFade()
            return

    def __handleCongrats(self, avatarChoice):
        if not self.acceptedBackground:
            return
        else:
            self.acceptedBackground.removeNode()
            self.acceptedScroll.removeNode()
            self.acceptedBanner.destroy()
            self.acceptedText.destroy()
            self.acceptedButton.destroy()
            self.applauseSound.stop()
            self.getApprovedMusic().stop()
            self.acceptedBackground = None
            del self.acceptedScroll
            del self.acceptedBanner
            del self.acceptedText
            del self.acceptedButton
            self.csm.sendAcknowledgeAvatarName(avatarChoice.id, lambda : self.loginFSM.request('waitForSetAvatarResponse', [avatarChoice, self.index]))
            return

    def handleNameRejected(self, avList, index):
        if self.rejectedBackground:
            return
        else:
            if not self.rejectedSound:
                self.rejectedSound = loader.loadSfx('phase_6/audio/sfx/Golf_Sad_Noise_Kicked_Off_Hole.ogg')
            base.playSfx(self.rejectedSound, volume=0.7)
            base.playMusic(self.getRejectedMusic(), looping=True, volume=0.9)
            self.rejectedBackground = GUIUtils.loadRandomGradient(render2d, (-1, 1, -1, 1), 0, 0.85, 0.4)
            self.rejectedText = DirectLabel(relief=None, text=TTLocalizer.NameShopNameRejected, text_scale=0.1, text_fg=(1, 1, 1, 1), text_shadow=(0, 0, 0, 1), pos=(0, 0, 0.1))
            self.rejectedButton = DirectButton(relief=None, image=Preloaded['okButton'], text=TTLocalizer.lOK, text_fg=(1, 1, 1, 1), text_shadow=(0, 0, 0, 1), scale=1.35, text_scale=0.05, text_pos=(0, -0.1), pos=(0, 0, -0.1), command=self.__handleReject, extraArgs=[avList, index])
            base.transitions.noFade()
            return

    def __handleReject(self, avList, index):
        if not self.rejectedBackground:
            return
        else:
            self.rejectedBackground.removeNode()
            self.rejectedText.destroy()
            self.rejectedButton.destroy()
            self.rejectedSound.stop()
            self.getRejectedMusic().stop()
            self.rejectedBackground = None
            del self.rejectedText
            del self.rejectedButton
            for av in avList:
                if av.position == index:
                    self.csm.sendAcknowledgeAvatarName(av.id, lambda : self.loginFSM.request('waitForAvatarList'))
                    return

            self.notify.error('Avatar rejected not found in avList. Index is: %s' % index)
            return

    def handleNoConnection(self, message, callback = None, color = (0.6, 0.85, 0.4), background = True):
        if self.noConnectionBackground:
            return
        else:
            base.playMusic(self.getNoConnectionMusic(), looping=True, volume=0.9)
            if background:
                self.noConnectionBackground = GUIUtils.loadRandomGradient(render2d, (-1, 1, -1, 1), *color)
            self.noConnectionText = DirectLabel(aspect2dp, relief=None, text=message, text_scale=0.09, text_fg=(1, 1, 1, 1), text_shadow=(0, 0, 0, 1), text_wordwrap=22)
            left, right, down, up = self.noConnectionText.node().getFrame()
            down = abs(down) / 2
            if not callback:
                callback = lambda : self.loginFSM.request('shutdown')
            self.noConnectionButton = DirectButton(aspect2dp, relief=None, image=Preloaded['okButton'], text=TTLocalizer.lOK, text_shadow=(0, 0, 0, 1), text_fg=(1, 1, 1, 1), text_scale=0.05, text_pos=(0, -0.1), pos=(0, 0, -down), command=callback)
            self.noConnectionText.setZ(down + 0.1)
            return

    def cleanupNoConnection(self):
        if not self.noConnectionBackground:
            return
        else:
            self.noConnectionBackground.removeNode()
            self.noConnectionText.destroy()
            self.noConnectionButton.destroy()
            self.getNoConnectionMusic().stop()
            self.noConnectionBackground = None
            del self.noConnectionText
            del self.noConnectionButton
            return
    ...
    def __handleAvatarChooserDone(self, done, extraArgs = []):
        self.index = self.avChoice.toonIndex
        av = self.avChoice.getAvatar(self.index)
        if done == 'exit':
            self.loginFSM.request('shutdown')
        elif done == 'chose':
            self.avChoice.exit()
            if av.approvedName != '':
                self.handleNameApproved(av)
                av.approvedName = ''
            elif av.rejectedName != '':
                av.rejectedName = ''
                self.handleNameRejected(self.avChoice.avList, self.index)
            else:
                self.loginFSM.request('waitForSetAvatarResponse', [av, self.index])
        elif done == 'create':
            self.loginFSM.request('createAvatar', [self.index])
        elif done == 'delete':
            self.loginFSM.request('waitForDeleteAvatarResponse', [av])
    ...
    def enterCreateAvatar(self, index):
        base.getMainTheme().stop()
        base.transitions.noFade()
        self.avCreate = MakeAToon.MakeAToon(self.loginFSM, self.avList, 'makeAToonComplete', index)
        self.avCreate.load()
        self.avCreate.enter()
        self.accept('makeAToonComplete', self.__handleMakeAToon, [self.avList, index])
    ...
    def __handleMakeAToon(self, avList, avPosition):
        ...
        if done == 'cancel':
            ...
            self.loginFSM.request('waitForSetAvatarResponse', [newPotAv, avPosition])
        else:
    ...
    def handleAvatarResponseMsg(self, avatarId, di):
        ...
        loader.beginBulkLoad('localAvatarPlayGame', OTPLocalizer.CREntering, 0, TTLocalizer.TIP_GENERAL, 0)
        localAvatar = LocalToon.LocalToon(self)
        localAvatar.dclass = dclass
        base.localAvatar = localAvatar
        base.localAvatar.chatMgr.reloadControls()
    ...
    def __requestSkipTutorial(self, hoodId, zoneId, avId):
        ...
        self.acceptOnce('skipTutorialDone', self.__handleSkipTutorialDone)
        messenger.send('requestSkipTutorial')

    def __handleSkipTutorialDone(self):
    ...
    def __handleStartTutorial(self, avId, zoneId):
        self.gameFSM.request('playGame', [CogtownCentral, zoneId, avId])
    ...
    def isFriendsMapPending(self):
        return time.time() > self.nextFriendsRequest

    def requeryFriends(self, task = None):
        self.nextFriendsRequest = 0
        self.sendGetFriendsListRequest()
        return Task.done

    def updateFriendsMapTime(self):
        self.nextFriendsRequest = time.time() + 2.5
        taskMgr.doMethodLater(2.5, self.requeryFriends, 'tcr-friends')

    def clearFriendsMapTime(self):
        self.nextFriendsRequest = 0
        taskMgr.remove('tcr-friends')

    def fillUpFriendsMap(self):
        if not self.isFriendsMapPending():
    ...
    def clearFriendState(self):
        self.friendsMap = {}
        self.friendsOnline = {}
        self.clearFriendsMapTime()

    def sendGetFriendsListRequest(self, task = None):
        self.updateFriendsMapTime()
    ...
    def handleGetFriendsList(self, resp):
        for toon in resp:
            doId, name, dnaString, petId = toon
            handle = FriendHandle.FriendHandle(doId, name, ToonDNA.ToonDNA(dnaString), petId)
            self.friendsMap[doId] = handle
    ...
    def handleFriendsOnline(self, doIds):
        for doId in doIds:
            self.friendsOnline[doId] = self.identifyAvatar(doId)
    ...
    def __fellAsleep(self):
        self.sendSetAvatarIdMsg(0, 0)
        if not hasattr(base.cr, 'lastDNA'):
            self.__handleAfkOk()
            return
        name, style = base.cr.lastDNA
        self.afkDialog = SleepScreen(aspect2d, name=name, command=self.__handleAfkOk, dnaString=style)
        self.handler = self.handleMessageType

    def __handleAfkOk(self):
        self.afkDialog.destroy()
        self.loginFSM.request('waitForAvatarList')

    def enableTransparentToons(self):
        if self.transparentToons:
            return
        for toon in self.toons:
            toon = self.doId2do.get(toon)
            if toon and toon.isPlayerControlled():
                self.__onToonGenerate(toon)

        self.accept('toonGenerated', self.__onToonGenerate)
        self.transparentToons = True

    def disableTransparentToons(self):
        if not self.transparentToons:
            return
        for toon in self.toons:
            toon = self.doId2do.get(toon)
            if not toon or not toon.isPlayerControlled():
                continue
            toon.clearColorScale()
            toon.setTransparency(False)

        self.ignore('toonGenerated')
        self.transparentToons = False

    def __onToonGenerate(self, toon):
        toon.setTransparency(True)
        toon.setColorScale(1, 1, 1, 0.1)