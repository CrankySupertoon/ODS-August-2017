# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.distributed.ToontownDistrictStats
...
class ToontownDistrictStats(DistributedObject.DistributedObject):
    ...
    def setEasterEgg(self, easterEgg):
        if self.districtId in self.cr.activeDistrictMap:
            self.cr.activeDistrictMap[self.districtId].easterEgg = easterEgg
            messenger.send('shardInfoUpdated')