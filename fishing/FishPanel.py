# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.fishing.FishPanel
...
class FishPanel(DirectFrame):
    ...
    def load(self, dump = False):
        ...
        closeImage = (buttons.find('**/CloseBtn_UP'), buttons.find('**/CloseBtn_DN'), buttons.find('**/CloseBtn_Rllvr'))
        self.cancel = DirectButton(parent=self, pos=(0.275, 0, -0.375), relief=None, state=DGG.NORMAL, image=closeImage, image_scale=(0.6, 1, 0.6), command=self.handleCancel)
        if dump:
            self.dump = DirectButton(parent=self, pos=(-0.275, 0, -0.375), relief=None, state=DGG.NORMAL, image=closeImage, image_color=(1, 0, 0, 1), image_scale=(0.6, 1, 0.6), text=('',
             TTLocalizer.FishGuiDumpSingle,
             TTLocalizer.FishGuiDumpSingle,
             ''), text_scale=0.04, text_pos=(0.04, -0.01), text_align=TextNode.ALeft, command=self.handleDump)
        buttons.removeNode()
    ...
    def handleDump(self):
        base.localAvatar.b_requestDumpFish()
        self.handleCancel()
