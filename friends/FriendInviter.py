# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.friends.FriendInviter
...
class FriendInviter(DirectFrame):
    ...
    def __init__(self, avId, avName, avDisableName):
        ...
        self.fsm = ClassicFSM.ClassicFSM('FriendInviter', [State.State('off', self.enterOff, self.exitOff),
         ...
         State.State('otherCog', self.enterOtherCog, self.exitOtherCog),
    ...
    def getTheName(self, upper = True):
        name = self.getName()
        if TTLocalizer.lThe not in name:
            name = '%s %s' % (TTLocalizer.lThe if upper else TTLocalizer.lThe.lower(), name)
        return name
    ...
    def enterGetNewFriend(self):
        self['text'] = OTPLocalizer.FriendInviterClickToon % len(base.localAvatar.friendsList)
        self.bCancel.show()
    ...
    def enterCheck(self):
        ...
        elif len(base.localAvatar.friendsList) >= MaxFriends:
    ...
    def enterTooMany(self):
        text = OTPLocalizer.FriendInviterTooMany
        name = self.toonName
        self['text'] = text % name
        self.bCancel.show()
    ...
    def enterAlready(self):
        self['text'] = OTPLocalizer.FriendInviterAlready % self.getName()
        self.bStop['text'] = OTPLocalizer.FriendInviterStopBeingFriends
        self.context = None
        self.bStop.show()
        self.bCancel.show()
        return
    ...
    def enterAskingCog(self):
        self['text'] = OTPLocalizer.FriendInviterAskingCog % self.getTheName(False)
    ...
    def enterEndFriendship(self):
        self['text'] = OTPLocalizer.FriendInviterEndFriendship % self.getName()
        self.context = None
        self.bYes.show()
        self.bNo.show()
        return
    ...
    def enterOtherCog(self):
        self['text'] = random.choice(TTLocalizer.FriendInviterOtherCogInvites) % (self.getTheName(), random.choice(TTLocalizer.FriendInviterOtherCogResponses))
        self.context = None
        self.bOk.show()
        return

    def exitOtherCog(self):
        self.bOk.hide()
