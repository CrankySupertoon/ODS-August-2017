# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.hood.Hood
...
class Hood(StateData.StateData):
    ...
    def doSpawnTitleText(self, text):
        ...
        self.titleText.show()
        self.titleText.setColor(*self.titleColor)
        self.titleText.clearColorScale()
        self.titleText.setFg(self.titleColor)
        Sequence(Wait(6.0), self.titleText.colorScaleInterval(0.5, (1.0, 1.0, 1.0, 0.0)), Func(self.hideTitleText)).start()
    ...
    def handleWaitForSetZoneResponse(self, requestStatus):
        ...
        if loaderName == 'safeZoneLoader':
            if not loader.inBulkBlock:
                loader.beginBulkLoad('hood', TTLocalizer.HeadingToPlayground, True, TTLocalizer.TIP_GENERAL, self.id)
            ...
            if not loader.inBulkBlock:
                ...
                toPhrase = StreetNames[branch][0]
                streetName = StreetNames[branch][-1]
                loader.beginBulkLoad('hood', TTLocalizer.HeadingToStreet % {'to': toPhrase,
                 'street': streetName}, True, TTLocalizer.TIP_STREET, branch)
            self.loadLoader(requestStatus)
            loader.endBulkLoad('hood')
