# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.hood.Place
...
class Place(StateData.StateData, FriendsListManager.FriendsListManager):
    ...
    def isTown(self):
        return ZoneUtil.getWhereName(self.zoneId, True) == 'street'
    ...
    def enterWalk(self, teleportIn = 0):
        ...
        self.walkStateData.fsm.request('walking')
        messenger.send('enterWalk')
        if self.isTown():
            base.localAvatar.questMap.acceptOnscreenHooks()
            base.localAvatar.questMap.start()
    ...
    def exitWalk(self):
        ...
        base.localAvatar.hideSitButton()
        if self.isTown():
    ...
    def enterSit(self):
        ...
        self.stopSitKey = base.getKey('up')
        self.accept(self.stopSitKey, self.fsm.request, extraArgs=['walk'])
    ...
    def __handleBookExitTeleport(self, requestStatus):
        if base.cr.miscManager:
            base.cr.miscManager.setDisconnectReason(ToontownGlobals.DisconnectBookExit)
    ...
    def handleEnterTunnel(self, requestStatus, collEntry):
        if localAvatar.hasActiveBoardingGroup():
            localAvatar.elevatorNotifier.showMe(TTLocalizer.BoardingCannotLeaveZone)
            self.kickPlayerFromTunnel(requestStatus['tunnelOrigin'])
            return
        self.requestLeave(requestStatus)

    def kickPlayerFromTunnel(self, tunnelOrigin):
    ...
    def enterTeleportOut(self, requestStatus, callback):
        if requestStatus.get('battle', False):
    ...
    def getEstateZoneAndGoHome(self, requestStatus):
        ...
        if 'skipEstateMgr' in requestStatus:
            self.goToEstate()
            return
        ...
        if avId != -1:
            if avId in base.cr.doId2do:
                ...
                if friend == None:
                    ...
                    base.localAvatar.d_teleportGiveup(avId)
                else:
    ...
    def enterPhone(self):
        base.localAvatar.b_setAnimState('neutral', 1)
        Emote.globalEmote.disableBody(base.localAvatar, 'enterPhone')
        self.accept('teleportQuery', self.handleTeleportQuery)
        base.localAvatar.setTeleportAvailable(1)
        base.localAvatar.laffMeter.start()
        base.localAvatar.obscureMoveFurnitureButton(1)
    ...
    def exitPhone(self):
        base.localAvatar.setTeleportAvailable(0)
        Emote.globalEmote.releaseBody(base.localAvatar, 'exitPhone')
        self.ignore('teleportQuery')
        base.localAvatar.laffMeter.stop()
        base.localAvatar.obscureMoveFurnitureButton(0)
        base.localAvatar.stopSleepWatch()
