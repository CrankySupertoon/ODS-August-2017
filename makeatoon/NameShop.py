# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.makeatoon.NameShop
...
from toontown.uberdog import ToontownNameCheck
...
class NameShop(StateData.StateData):
    ...
    def __init__(self, makeAToon, doneEvent, avList, index):
        ...
        self.fsm = ClassicFSM.ClassicFSM('NameShop', [State.State('Init', self.enterInit, self.exitInit, ['PickAName']),
         ...
         State.State('TypeAName', self.enterTypeANameState, self.exitTypeANameState, ['PickAName', 'Approval', 'Rejected']),
    ...
    def nameIsValid(self, name):
        return NameCheck.checkName(name, ToontownNameCheck.getExtraChecks(), font=self.nameEntry.getFont())
    ...
    def serverCreateAvatar(self, skipTutorial = False):
        ...
        if not self.avExists or self.avExists and self.avId == 'deleteMe':
            base.cr.csm.sendCreateAvatar(style, self.index, self.toon.hpLimit, self.toon.hardcoreMode)
