# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.pets.PetshopGUI
...
class PetshopGUI(DirectObject):
    ...
    class GoHomeDlg(DirectFrame):
        ...
        def __init__(self, doneEvent):
            ...
            self.bYes = DirectButton(self, image=(buttons.find('**/ChtBx_OKBtn_UP'), buttons.find('**/ChtBx_OKBtn_DN'), buttons.find('**/ChtBx_OKBtn_Rllvr')), relief=None, text=TTLocalizer.lYes, text_scale=0.05, text_pos=(0.0, -0.1), pos=(-0.15, 0.0, -0.1), command=lambda : messenger.send(doneEvent, [1]))
            self.bNo = DirectButton(self, image=(buttons.find('**/CloseBtn_UP'), buttons.find('**/CloseBtn_DN'), buttons.find('**/CloseBtn_Rllvr')), relief=None, text=TTLocalizer.lNo, text_scale=0.05, text_pos=(0.0, -0.1), pos=(0.15, 0.0, -0.1), command=lambda : messenger.send(doneEvent, [0]))
            buttons.removeNode()
            gui.removeNode()
            return
    ...
    class AdoptPetDlg(DirectFrame):
        ...
        def __init__(self, doneEvent, petSeed, petNameIndex):
            ...
            self.petModel.setScale(0.125)
            self.petModel.enterNeutralHappy()
            self.moneyDisplay = DirectLabel(parent=self, relief=None, text=str(base.localAvatar.getBankMoney()), text_scale=0.075, text_fg=(0.95, 0.95, 0, 1), text_shadow=(0, 0, 0, 1), text_pos=(0.225, 0.33), text_font=ToontownGlobals.getSignFont())
        ...
        def __moneyChange(self, money):
            self.moneyDisplay['text'] = str(base.localAvatar.getBankMoney())
    ...
    class ChoosePetDlg(DirectFrame):
        ...
        def __init__(self, doneEvent, petSeeds):
            ...
            self.moneyDisplay = DirectLabel(parent=self, relief=None, text=str(base.localAvatar.getBankMoney()), text_scale=0.1, text_fg=(0.95, 0.95, 0, 1), text_shadow=(0, 0, 0, 1), text_pos=(0.34, 0.12), text_font=ToontownGlobals.getSignFont())
            self.accept(localAvatar.uniqueName('moneyChange'), self.__moneyChange)
            self.accept(localAvatar.uniqueName('bankMoneyChange'), self.__moneyChange)
            self.petView = self.attachNewNode('petView')
        ...
        def showPet(self):
            ...
            if self.petCost[self.curPet] > base.localAvatar.getBankMoney():
    ...
    def doDialog(self, nDialog):
        ...
        if nDialog == Dialog_MainMenu:
            ...
            self.dialog = FishSellGUI.FishSellGUI(self.__handleFishSellDlg)
    ...
    def __handleFishSellDlg(self, exitVal):
        if exitVal == 0:
            ...
            self.destroyDialog()
            messenger.send(self.eventDict['fishSold'], [exitVal])
