# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.quest.QuestMap
...
from toontown.suit import SuitPlannerBase, SuitDNA
...
class QuestMap(DirectFrame):
    ...
    def __init__(self, av, **kw):
        ...
        self.cogInfoFrame['geom_pos'] = (0, 0, -0.5)
        self.cogInfoFrame['geom_scale'] = (6, 1, 3)
    ...
    def load(self):
        ...
        for dept in SuitDNA.suitDepts:
            icon = icons.find(SuitDNA.suitDeptModelPaths[dept])
            info = DirectLabel(self.cogInfoFrame, text_fg=(0.2, 0.2, 0.2, 1), text_pos=(1.2, -0.2), text_scale=0.8, text='', geom=icon, geom_pos=(-0.2, 0, 0), geom_scale=0.8, relief=None)
            info.setPos(iconPos[dept])
    ...
    def updateCogInfo(self):
        ...
        if currPercentage is None:
            ...
            for i, percentage in enumerate(currPercentage):
                self.info[i]['text'] = '%s%%' % percentage
    ...
    def getBuildingTrack(self, block):
        zoneId = base.cr.playGame.dnaStore.getZoneFromBlockNumber(block)
        if zoneId and hasattr(base.cr, 'currSuitPlanner') and base.cr.currSuitPlanner:
            return base.cr.currSuitPlanner.getBuildingTrack(zoneId)

    def putBuildingMarker(self, pos, mapIndex = None, track = False, zoneId = None, special = False):
        if zoneId in self.buildingMarkers:
            return
        else:
            if isinstance(pos, int):
                track = self.getBuildingTrack(pos)
                pos, hpr = base.cr.playGame.dnaStore.getBlockPosHpr(pos)
                if not pos or not hpr:
                    return
            ...
            marker['image'] = iconNP
            marker['image_color'] = (0.45, 0.45, 0.45, 1) if track else ((1, 0, 0, 1) if not special else (0.75, 0, 0.75, 1))
            marker['image_scale'] = 6
            marker.setScale(0.05)
            relX, relY = self.transformAvPos(pos)
            marker.setPos(relX, 0, relY)
            self.buildingMarkers[zoneId] = marker
            iconNP.removeNode()
            gui.removeNode()
            return

    def updateQuestInfo(self):
        for marker in self.buildingMarkers.values():
            marker.destroy()
        ...
        for i, questDesc in enumerate(self.av.quests):
            ...
            for blockIndex in xrange(base.cr.playGame.dnaStore.getNumBlockNumbers()):
                ...
                if npcZoneId == interiorZoneId:
                    ...
                    self.putBuildingMarker(blockNumber, mapIndex=mapIndex, zoneId=zoneId)

        special = 1
        for blockIndex in xrange(base.cr.playGame.dnaStore.getNumBlockNumbers()):
            blockNumber = base.cr.playGame.dnaStore.getBlockNumberAt(blockIndex)
            if blockNumber in taskBlocks:
                continue
            if self.getBuildingTrack(blockNumber):
                self.putCogBuildingMarker(blockNumber)
            elif base.cr.playGame.dnaStore.getBlockBuildingType(blockNumber) == 'toptoons':
                zoneId = base.cr.playGame.dnaStore.getZoneFromBlockNumber(blockNumber)
                self.putBuildingMarker(blockNumber, mapIndex=special, zoneId=zoneId, special=True)
                special += 1

    def putCogBuildingMarker(self, blockNumber):
        zoneId = base.cr.playGame.dnaStore.getZoneFromBlockNumber(blockNumber)
        if zoneId in self.buildingMarkers:
            return
        else:
            pos, hpr = base.cr.playGame.dnaStore.getBlockPosHpr(blockNumber)
            if not pos or not hpr:
                return
            marker = DirectLabel(parent=self.container, text='', text_pos=(-0.05, -0.15), text_fg=(1, 1, 1, 1), relief=None)
            gui = loader.loadModel('phase_3/models/gui/cog_icons')
            icon = gui.find(SuitDNA.suitDeptModelPaths[self.getBuildingTrack(blockNumber)])
            iconNP = aspect2d.attachNewNode('iconNP')
            icon.reparentTo(iconNP)
            icon.setX(-12.0792 / 30.48)
            icon.setZ(-9.7404 / 30.48)
    ...
    def update(self, task):
        ...
        i = 0
        for buildingMarker in self.buildingMarkers.values():
            if not buildingMarker:
                return
    ...
    def updateMap(self):
        if self.av:
            ...
            if hoodId not in ToontownGlobals.dnaMap:
                self.stop()
                return
