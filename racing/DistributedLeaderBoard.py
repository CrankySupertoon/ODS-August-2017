# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.racing.DistributedLeaderBoard
...
class DistributedLeaderBoard(DistributedObject.DistributedObject):
    ...
    def setDisplay(self, track, type, results):
        if track not in TTLocalizer.KartRace_TrackNames or len(TTLocalizer.RecordPeriodStrings) <= type:
            return
    ...
    def display(self, pTrackTitle = 'Track Title', pPeriodTitle = 'Period Title', pLeaderList = []):
        ...
        for i in xrange(10):
            if i >= len(pLeaderList):
                ...
                self.nameTextNodes[i].setText(pLeaderList[i][1][:22])
                self.timeTextNodes[i].setText(TTLocalizer.convertSecondsToTimer(pLeaderList[i][0] / 100.0))
