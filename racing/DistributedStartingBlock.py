# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.racing.DistributedStartingBlock
from panda3d.core import CollideMask, CollisionNode, CollisionSphere, NodePath, Point3, TextNode, Vec4
...
class DistributedStartingBlock(DistributedObject.DistributedObject, FSM):
    ...
    def setMovie(self, timestamp, mode):
        self.notify.debugStateCall(self)
        self.finishMovie()
        if self.avId == 0:
            return
        ...
        if mode == KartGlobals.ENTER_MOVIE:
            self.request('EnterMovie', timestamp)
        elif mode == KartGlobals.EXIT_MOVIE:
            self.request('ExitMovie', timestamp)
    ...
    def generateToonJumpTrack(self):
        ...
        def getToonJumpTrack(av, kart):
            ...
            def getJumpDest(av = av, node = kart.toonNode):
                dest = node.getPos(av.getParent())
                return dest

            def getJumpHpr(av = av, node = kart.toonNode):
        ...
        jumpTrack = Sequence(Parallel(toonJumpTrack, Sequence(Wait(1), toonSitTrack)), Func(self.av.setPosHpr, 0, 0.45, -0.25, 0, 0, 0), Func(self.av.reparentTo, self.kart.toonNode))
        return jumpTrack
    ...
    def enterEnterMovie(self, timestamp):
        ...
        if self.av is not None and self.localToonKarting:
            ...
            self.finishMovie()
            self.movieTrack = Sequence(Parallel(cameraTrack, toonTrack), Parallel(SoundInterval(kartAppearSfx), Sequence(kartTrack, jumpTrack, engineStartTrack, Func(self.d_movieFinished), Wait(0.5), Func(self.makeGui), Func(self.showGui), Func(self.request, 'Waiting'))))
            self.exitRequested = False
        else:
            self.finishMovie()
            self.movieTrack = Sequence(toonTrack, kartTrack, jumpTrack)
        self.movieTrack.start(timestamp)
        return
    ...
    def enterExitMovie(self, timestamp):
    ...
    def getTrack(self):
        return self.getPad().trackId

    def bulkLoad(self):
        track = self.getPad().trackId
        base.loader.beginBulkLoad('atRace', TTLocalizer.HeadingToFactory % TTLocalizer.KartRace_TrackNames[track], True, TTLocalizer.TIP_KARTING, track)
...
class DistributedViewingBlock(DistributedStartingBlock):
    ...
    def enterEnterMovie(self, timestamp):
        ...
        if self.av is not None and self.localToonKarting:
            cameraTrack = self.generateCameraMoveTrack()
            self.finishMovie()
            self.movieTrack = Sequence(cameraTrack, kartTrack, jumpTrack, Func(self.makeGui), Func(self.showGui), Func(self.countdown), Func(self.request, 'Waiting'), Func(self.d_movieFinished))
        else:
            self.finishMovie()
            self.movieTrack = Sequence(toonTrack, kartTrack, jumpTrack)
        self.movieTrack.start(timestamp)
