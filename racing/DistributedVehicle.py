# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.racing.DistributedVehicle
...
from panda3d.core import BitMask32, CardMaker, CollideMask, CollisionHandler, CollisionHandlerEvent, CollisionHandlerGravity, CollisionHandlerQueue, CollisionNode, CollisionRay, CollisionSphere, CollisionTraverser, ConfigVariable, ConfigVariableBool, Lens, Mat3, NodePath, Point3, Vec3, Vec4, lookAt
...
class DistributedVehicle(DistributedSmoothNode.DistributedSmoothNode, Kart.Kart, FSM.FSM):
    ...
    turnRatio = 1.0 / 0.025 * math.sqrt(1.5)
    accelerationMult = 35
    accelerationBase = 20
    if proRacer:
        accelerationMult = 35
    ...
    def __init__(self, cr):
        ...
        self.actionKey = base.getKey('pickup')
    ...
    def announceGenerate(self):
        ...
        self.forward.reparentTo(self.geom)
    ...
    def __enableCollisions(self):
        ...
        self.cRays.reparentTo(self.geom)
    ...
    def setupDriftParticles(self):
        smokeMount = self.geom.attachNewNode('Smoke Effect')
        backLeft = smokeMount.attachNewNode('Back Left Smokemount')
        backLeft.setPos(self.geom.find('**/' + self.wheelData[self.LRWHEEL]['node']).getPos() + Vec3(-0.25, -1.0, -0.5))
        backRight = smokeMount.attachNewNode('Back Right Smokemount')
        backRight.setPos(self.geom.find('**/' + self.wheelData[self.RRWHEEL]['node']).getPos() + Vec3(0.25, -1.0, -0.5))
    ...
    def setupSparkParticles(self):
        ...
        endPts = KartDict[bodyType][KartInfo.sparkEndPoints]
        self.sparkMount = self.geom.attachNewNode('Spark Effect')
        left = self.sparkMount.attachNewNode('Left Sparkmount')
        left.setPos((self.geom.find('**/' + self.wheelData[self.LFWHEEL]['node']).getPos() + self.geom.find('**/' + self.wheelData[self.LRWHEEL]['node']).getPos()) / 2.0 + Vec3(0.0, -1.0, 1.0))
        left.setScale(0.5)
        right = self.sparkMount.attachNewNode('Right Sparkmount')
        right.setPos((self.geom.find('**/' + self.wheelData[self.RFWHEEL]['node']).getPos() + self.geom.find('**/' + self.wheelData[self.RRWHEEL]['node']).getPos()) / 2.0 + Vec3(0.0, -1.0, 1.0))
        right.setScale(0.5)
    ...
    def exitControlled(self):
        ...
        if self.toon and not self.toon.isDisabled() and not self.toon.isEmpty():
            ...
            self.toon.setPosHpr(self.geom, 0, 8, 0, 0, 0, 0)
            for feet in self.toon.getPieces(('legs', 'feet')):
                feet.show()
    ...
    def __animate(self):
        ...
        if not self.localVehicle:
            if not base.localAvatar or base.localAvatar.isEmpty() or self.isEmpty():
                return
            dist = (self.getPos() - localAvatar.getPos()).length()
        if self.localVehicle:
            if self.lifter.isOnGround() and not self.hasNoAccessories():
                if self.offGround > 10:
                    kart = self.geom.find('**/main*')
    ...
    def __watchControls(self, task):
        ...
        if self.stopped:
            ...
            if self.proRacer and self.skidding:
    ...
    def enableControls(self):
        self.canRace = True
        self.actionKey = base.getKey('pickup')
        self.accept(self.actionKey, self.__controlPressed)
        self.accept(self.actionKey + '-up', self.__controlReleased)
    ...
    def disableControls(self):
        self.arrowVert = 0
        self.arrowHorz = 0
        self.ignore(self.actionKey)
        self.ignore(self.actionKey + '-up')
        self.ignore('tab')
    ...
    def dropOnMe(self, timestamp):
        ...
        flattenMe = self.geom.scaleInterval(0.08, Vec3(1, 1, 0.1))
        unFlatten = Sequence(self.geom.scaleInterval(0.2, Vec3(1, 1, 2)), self.geom.scaleInterval(0.1, 1))
    ...
    def lookUp(self):
        if self.toon:
            for head in self.toon.getHeadParts():
                head.setP(90)
    ...
    def hitBanana(self):
        if self.wipeOut:
            self.wipeOut.pause()
            spinAnim = LerpFunc(self.spinCar, fromData=self.geom.getH(), toData=360, duration=1)
        else:
    ...
    def hitPie(self):
        ...
        if self.wipeOut:
            self.wipeOut.pause()
            spinAnim = LerpFunc(self.spinCar, fromData=self.geom.getH(), toData=1080, duration=0.5)
        else:
