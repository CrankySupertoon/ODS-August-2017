# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.shtiker.FishPage
...
class FishPage(ShtikerPage.ShtikerPage):
    ...
    def enter(self):
        ...
        self.accept('refreshFishingLure', self.updatePage)
        self.accept('refreshFishingRod', self.updatePage)
        self.accept('hideFishPanel', self.__hideFishPanel)
        self.accept('showFishPanel', self.__showFishPanel)
    ...
    def createFishPicker(self):
        if not hasattr(self, 'picker'):
            ...
            self.picker.setPos(-0.6, 0, 0.1)
            self.picker.setScale(0.95)
            self.rod = DirectLabel(parent=self.picker, relief=None, text='', text_align=TextNode.ALeft, text_scale=0.06, pos=(0.9, 0, -0.6))
        return
    ...
    def updatePage(self):
        ...
        if hasattr(self, 'rod'):
            ...
            lure = base.localAvatar.fishingLure
            rodName = TTLocalizer.FishingRodNameDict[rod]
            lureName = TTLocalizer.FishingLureColors[lure]
            lureSpeed = FishGlobals.Lure2Speed[lure]
            rodWeightRange = FishGlobals.getRodWeightRange(rod)
            self.rod['text'] = TTLocalizer.FishPageRodInfo % (rodName,
             lureName,
             lureSpeed,
             rodWeightRange[0],
             rodWeightRange[1])
            if hasattr(self, 'rodPic'):
                self.rodPic.removeNode()
            self.rodPic = loader.loadModel(FishGlobals.RodFileDict[rod])
            self.rodPic.reparentTo(self.picker)
            self.rodPic.setPos(0.8, 0, -0.45)
            self.rodPic.setHpr(-90, 55, -98)
            self.rodPic.setScale(0.18)
            if base.localAvatar.fishTank:
                self.rodPic.hide()
    ...
    def __hideFishPanel(self):
        if hasattr(self, 'rodPic'):
            self.rodPic.show()

    def __showFishPanel(self):
        if hasattr(self, 'rodPic'):
            self.rodPic.hide()
