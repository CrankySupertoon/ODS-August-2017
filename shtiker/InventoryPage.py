# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.shtiker.InventoryPage
...
class InventoryPage(ShtikerPage.ShtikerPage):
    ...
    def uniqueName(self, name):
        return 'InventoryPage-%s-%s' % (id(self), name)
    ...
    def load(self):
        ...
        self.bankMoneyDisplay = DirectLabel(self, relief=None, pos=(0.35, 0, -0.1), scale=0.6, image=Preloaded['beanBank'], text=str(base.localAvatar.getBankMoney()), text_align=TextNode.ARight, text_scale=0.11, text_fg=(0.95, 0.95, 0, 1), text_shadow=(0, 0, 0, 1), text_pos=(0.75, -0.81), text_font=ToontownGlobals.getSignFont())
        self.moreXpLabel = DirectLabel(self, relief=None, text=TTLocalizer.MoreXpLabel, text_scale=0.08, text_fg=(1, 0.5, 0, 1), text_shadow=(0, 0, 0, 1), text_align=TextNode.ALeft, pos=(-0.8, 0, 0.625))
        self.moreXpTimer = DirectLabel(self, relief=None, text='', text_scale=0.08, text_fg=(1, 0.5, 0, 1), text_shadow=(0, 0, 0, 1), text_align=TextNode.ARight, pos=(0.8, 0, 0.625))
    ...
    def createMeritBars(self):
        ...
        for i in xrange(len(SuitDNA.suitDepts)):
            self.meritBars.append(DirectWaitBar(parent=self.trackInfo, relief=DGG.SUNKEN, frameSize=(-1.2, 1.2, -0.15, 0.15), borderWidth=(0.02, 0.02), scale=0.56, text='', text_scale=0.18, text_fg=(0, 0, 0, 1), text_align=TextNode.ALeft, text_pos=(-1.16, -0.05), pos=(0, 0, 0.38 - 0.19 * i), frameColor=(DisguisePage.DeptColors[i][0] * 0.7,
             DisguisePage.DeptColors[i][1] * 0.7,
             DisguisePage.DeptColors[i][2] * 0.7,
             1), barColor=(DisguisePage.DeptColors[i][0] * 0.8,
             DisguisePage.DeptColors[i][1] * 0.8,
             DisguisePage.DeptColors[i][2] * 0.8,
             1)))
    ...
    def enter(self):
        ...
        self.updateDeadline()
        taskMgr.doMethodLater(1, self.updateDeadline, self.uniqueName('updateDeadline'))
    ...
    def exit(self):
        ...
        taskMgr.remove(self.uniqueName('updateDeadline'))
        return
    ...
    def hideDeadline(self):
        self.moreXpLabel.hide()
        self.moreXpTimer.hide()

    def showDeadline(self):
        self.moreXpLabel.show()
        self.moreXpTimer.show()

    def updateDeadline(self, task = None):
        if not base.localAvatar.hasMoreXp():
            self.hideDeadline()
            return
        seconds = base.localAvatar.getMoreXpDeadlineSeconds()
        if seconds <= 0:
            self.hideDeadline()
            return
        self.moreXpTimer['text'] = TTLocalizer.getHumanTimer(seconds, 5)
        self.showDeadline()
        if task:
            return task.again