# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.shtiker.MapPage
...
class MapPage(ShtikerPage.ShtikerPage):
    ...
    def load(self):
        ...
        for hood in ToontownGlobals.Hoods:
            if hood not in [ToontownGlobals.GolfZone, ToontownGlobals.ForestsEnd]:
        ...
        self.safeZoneButton = Button(parent=self.map, image_scale=(1.3, 1.1, 1.1), pos=buttonLoc, text=TTLocalizer.MapPageBackToPlayground, text_scale=TTLocalizer.MPsafeZoneButton, text_pos=(0, -0.02), command=self.backToSafeZone)
        self.goHomeButton = Button(parent=self.map, image_scale=(0.66, 1.1, 1.1), pos=(0.15, 0, -0.74), text=TTLocalizer.MapPageGoHome, text_scale=TTLocalizer.MPgoHomeButton, text_pos=(0, -0.02), command=self.goHome)
        self.goHomeButton.hide()
        self.hoodLabel = Label(parent=self.map, pos=(-0.43, 0, -0.726), text_scale=TTLocalizer.MPhoodLabel, text_pos=(0, 0), text_align=TextNode.ACenter, text_wordwrap=TTLocalizer.MPhoodLabelWordwrap)
        self.hoodLabel.hide()
    ...
    def enter(self):
        ...
        if base.localAvatar.lastHood >= ToontownGlobals.BossbotHQ and base.localAvatar.lastHood <= ToontownGlobals.TechbotServerRoomIntC:
