# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.shtiker.PhotoAlbumPage
from panda3d.core import TextNode, Texture, Vec4
import ShtikerPage
import ShtikerBook
...
class PhotoAlbumPage(ShtikerPage.ShtikerPage):
    ...
    def load(self):
        ...
        self.renameButton = Button(parent=self, image_scale=1, pos=(0.4, 0, -0.35), text=TTLocalizer.PhotoPageAddName, text_scale=0.06, command=self.renameImage, state=DGG.DISABLED)
        self.directoryButton = Button(parent=self, image_scale=(1.3, 0, 1), pos=(0.46, 0, -0.5), text=TTLocalizer.PhotoPageDirectory, text_scale=0.06, command=self.openPhotoDirectory)
        self.deleteButton = DirectButton(parent=self, image=Preloaded['trashcan'], text=('', TTLocalizer.AvatarChoiceDelete, TTLocalizer.AvatarChoiceDelete), text_fg=(1, 1, 1, 1), text_shadow=(0, 0, 0, 1), text_scale=0.1, text_pos=(0, -0.1), text_font=ToontownGlobals.getInterfaceFont(), textMayChange=0, relief=None, pos=(0.68, 0, -0.33), scale=0.4, state=DGG.DISABLED, command=self.deleteImage)
        self.scrollList = DirectScrolledList(parent=self, relief=None, forceHeight=0.07, pos=(-0.5, 0, 0), incButton_image=Preloaded['scrollBlueArrow'], incButton_relief=None, incButton_scale=(1.3, 1.3, -1.3), incButton_pos=(0.08, 0, -0.6), incButton_image3_color=Vec4(1, 1, 1, 0.2), decButton_image=Preloaded['scrollBlueArrow'], decButton_relief=None, decButton_scale=(1.3, 1.3, 1.3), decButton_pos=(0.08, 0, 0.52), decButton_image3_color=Vec4(1, 1, 1, 0.2), itemFrame_pos=(-0.237, 0, 0.41), itemFrame_scale=1.0, itemFrame_relief=DGG.SUNKEN, itemFrame_frameSize=(-0.05, 0.66, -0.98, 0.07), itemFrame_frameColor=(0.85, 0.95, 1, 1), itemFrame_borderWidth=(0.01, 0.01), numItemsVisible=13, items=[])
        ...
        self.bCancel = DirectButton(parent=self.renamePanel, image=Preloaded['closeButton'], relief=None, text=TTLocalizer.PhotoPageCancel, text_scale=0.05, text_pos=(0.0, -0.1), pos=(0.0, 0.0, -0.1), command=self.renameCancel)
        ...
        self.dOk = DirectButton(parent=self.deletePanel, image=Preloaded['okButton'], relief=None, text=TTLocalizer.PhotoPageConfirm, text_scale=0.05, text_pos=(0.0, -0.1), pos=(-0.1, 0.0, -0.1), command=self.deleteConfirm)
        self.dCancel = DirectButton(parent=self.deletePanel, image=Preloaded['closeButton'], relief=None, text=TTLocalizer.PhotoPageCancel, text_scale=0.05, text_pos=(0.0, -0.1), pos=(0.1, 0.0, -0.1), command=self.deleteCancel)
        ...
        self.bClose = DirectButton(parent=self.errorPanel, image=Preloaded['closeButton'], relief=None, text=TTLocalizer.PhotoPageClose, text_scale=0.05, text_pos=(0.0, -0.1), pos=(0.0, 0.0, -0.1), command=self.errorConfirm)
        self.errorPanel.hide()
        self.tip = DirectLabel(parent=self, relief=None, text=TTLocalizer.PhotoPageTutorial, text_scale=0.08, pos=(0.0, 0.0, 0.25), text_fg=(0.4, 0.3, 0.2, 1), text_wordwrap=18, text_align=TextNode.ACenter)
        self.leftArrow = DirectButton(parent=self, relief=None, image=Preloaded['blueArrow'], image3_color=Vec4(1, 1, 1, 0.5), scale=(-1.0, 1.0, 1.0), pos=(0.15, 0, -0.21), command=self.offsetPhoto, extraArgs=[-1])
        self.rightArrow = DirectButton(parent=self, relief=None, image=Preloaded['blueArrow'], image3_color=Vec4(1, 1, 1, 0.5), pos=(0.75, 0, -0.21), command=self.offsetPhoto, extraArgs=[1])
        return
    ...
    def renameDialog(self, str):
        if os.path.isfile(self.photoPath + self.selectedFileName):
            ...
            return 1
        self.renameCancel()
        self.errorPanel['text'] = TTLocalizer.PhotoPageDeletedError
    ...
    def deleteConfirm(self):
        if os.path.isfile(self.photoPath + self.selectedFileName):
            ...
            self.deleteCancel()
            self.errorPanel['text'] = TTLocalizer.PhotoPageDeletedAlreadyError
            self.errorPanel.show()
            self.updateScrollList()
            return
    ...
    def updateScrollList(self):
        ...
        if self.photos.keys():
            ...
            self.tip.hide()
            self.directoryButton.setX(0.46)
    ...
    def offsetPhoto(self, offset):
        try:
            self.chosePhoto(self.getPhotos()[self.photoIndex])
            self.photoIndex += offset
        except:
            self.photoIndex = 0