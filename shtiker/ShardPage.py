# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.shtiker.ShardPage
from panda3d.core import ButtonMap, NodePath, TextNode, Vec4
...
class ShardPage(ShtikerPage.ShtikerPage):
    ...
    def regenerateScrollList(self):
        ...
        self.scrollList = DirectScrolledList(parent=self, relief=None, pos=(-0.51, 0, 0), incButton_image=(self.gui.find('**/FndsLst_ScrollUp'),
         ...
         self.gui.find('**/FndsLst_ScrollUp')), incButton_relief=None, incButton_scale=(self.arrowButtonScale, self.arrowButtonScale, -self.arrowButtonScale), incButton_pos=(self.buttonXstart + 0.005, 0, self.itemFrameZorigin - 1.0), incButton_image3_color=Vec4(1, 1, 1, 0.2), decButton_image=(self.gui.find('**/FndsLst_ScrollUp'),
         self.gui.find('**/FndsLst_ScrollDN'),
         self.gui.find('**/FndsLst_ScrollUp_Rllvr'),
         self.gui.find('**/FndsLst_ScrollUp')), decButton_relief=None, decButton_scale=(self.arrowButtonScale, self.arrowButtonScale, self.arrowButtonScale), decButton_pos=(self.buttonXstart, 0.0025, self.itemFrameZorigin + 0.13), decButton_image3_color=Vec4(1, 1, 1, 0.2), itemFrame_pos=(self.itemFrameXorigin, 0, self.itemFrameZorigin), itemFrame_scale=1.0, itemFrame_relief=DGG.SUNKEN, itemFrame_frameSize=(self.listXorigin,
    ...
    def makeShardButton(self, shardId, groupAvCount, shardName, shardPop):
        ...
        shardButtonR = DirectButton(parent=shardButtonParent, relief=None, image=button, image_scale=(0.3, 1, 0.3), image2_scale=(0.35, 1, 0.35), image_color=self.getPopColor(shardPop), pos=(0.58, 0, 0.0125), text=popText, text_scale=0.06, text_align=TextNode.ARight, text_pos=(-0.14, -0.0165), text_fg=Vec4(0, 0, 0, 1), text3_fg=self.textDisabledColor, text1_bg=self.textRolloverColor, text2_bg=self.textRolloverColor, textMayChange=1, command=self.reloadRightBrain)
        model.removeNode()
        button.removeNode()
        invasionMarker = NodePath('InvasionMarker-%s' % shardId)
        invasionMarker.reparentTo(shardButtonParent)
        buttonTuple = (shardButtonParent,
         shardButtonR,
         shardButtonL,
         invasionMarker)
        shardButtonL['extraArgs'] = extraArgs = [shardPop,
         shardName,
         groupAvCount,
         shardId,
         buttonTuple]
        shardButtonR['extraArgs'] = extraArgs = [shardPop,
         shardName,
         groupAvCount,
         shardId,
         buttonTuple]
    ...
    def makeGroupButton(self, shardId, group, population):
        ...
        groupButtonR = DirectButton(parent=groupButtonParent, relief=None, pos=(0.58, 0, -0.0125), text=str(population), text_scale=0.055, text_align=TextNode.ACenter, command=self.joinGroup, extraArgs=[group, shardId], text_pos=(-0.00575, -0.0125), text_fg=Vec4(0, 0, 0, 1), text3_fg=Vec4(0, 0, 0, 1), text1_bg=self.textRolloverColor, text2_bg=self.textRolloverColor)
        return (groupButtonParent, groupButtonL, groupButtonR)

    def joinGroup(self, group, shardId):
        hoodId = ZoneUtil.getHoodId(group)
        shardName = base.cr.activeDistrictMap[shardId].name
        hoodName = TTLocalizer.GlobalStreetNames[hoodId]
        teleportAccess = base.localAvatar.hasTeleportAccess(hoodId)
        if teleportAccess:
        ...
        self.acceptOnce('groupDialogDone', self.__handleGroupDialog, extraArgs=[hoodId if teleportAccess else base.localAvatar.lastHood, shardId])
    ...
    def __handleGroupDialog(self, hoodId, shardId):
        response = self.groupDialog.doneStatus
        self.cleanupGroupDialog()
        if response == 'ok':
            self.requestTeleport(hoodId, shardId)
    ...
    def reloadRightBrain(self, shardPop, shardName, groupAvCount, shardId, buttonTuple):
        self.currentRightBrain = (shardPop,
         shardName,
         shardId,
         buttonTuple)
        ...
        for i, group in enumerate(ToontownGlobals.GROUP_ZONES):
            districtAvCount = groupAvCount[i] if len(groupAvCount) > i else 0
            btuple = self.makeGroupButton(shardId, group, districtAvCount)
            if ZoneUtil.getHoodId(base.localAvatar.zoneId) == ZoneUtil.getHoodId(group):
                btuple[1]['state'] = DGG.DISABLED
                btuple[2]['state'] = DGG.DISABLED
            self.shardGroups.append(btuple[0])
            self.shardText.append(btuple[2])

        buttonImage = (self.gui.find('**/FndsLst_ScrollUp'),
         self.gui.find('**/FndsLst_ScrollDN'),
         self.gui.find('**/FndsLst_ScrollUp_Rllvr'),
         self.gui.find('**/FndsLst_ScrollUp'))
        self.districtGroups = DirectScrolledList(parent=districtInfoNode, relief=None, pos=(0.38, 0, -0.34), incButton_image=buttonImage, incButton_relief=None, incButton_scale=(self.arrowButtonScale, self.arrowButtonScale, -self.arrowButtonScale), incButton_pos=(self.buttonXstart + 0.005, 0, -0.125), incButton_image3_color=Vec4(1, 1, 1, 0.2), decButton_image=buttonImage, decButton_relief=None, decButton_scale=(self.arrowButtonScale, self.arrowButtonScale, self.arrowButtonScale), decButton_pos=(self.buttonXstart, 0.0025, 0.445), decButton_image3_color=Vec4(1, 1, 1, 0.2), itemFrame_pos=(self.itemFrameXorigin, 0, self.itemFrameZorigin), itemFrame_scale=1.0, itemFrame_relief=DGG.SUNKEN, itemFrame_frameSize=(self.listXorigin,
         ...
         (self.listZorigin + self.listFrameSizeZ) / 2.1), itemFrame_frameColor=(0.85, 0.95, 1, 1), itemFrame_borderWidth=(0.01, 0.01), numItemsVisible=7, forceHeight=0.065, items=self.shardGroups)
        return
    ...
    def requestTeleport(self, hood, shardId):
        hoodId = ZoneUtil.getHoodId(hood)
        try:
