# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.shtiker.ShtikerBook
from panda3d.core import CardMaker, NodePath, TextNode, Texture, Vec4
...
class ShtikerBook(DirectFrame, StateData.StateData):
    ...
    def __init__(self, doneEvent):
        DirectFrame.__init__(self, frameColor=(0.05, 0.15, 0.4, 1), frameSize=(-10, 10, -10, 10))
        ...
        self.pageOrder = [TTLocalizer.OptionsPageTitle,
         ...
         TTLocalizer.EffectsPageTitle,
         TTLocalizer.StatPageTitle,
         TTLocalizer.BadgePageTitle]
        return
    ...
    def enter(self):
        ...
        for element in (self.bookOpenButton, self.bookCloseButton, base.localAvatar.laffMeter):
            element.setBin('gui-popup', 0)
    ...
    def load(self):
        ...
        for button in (self.bookOpenButton, self.bookCloseButton):
            button.hide()
    ...
    def addPageTab(self, page, pageIndex, pageName = 'Page'):
        ...
        if pageName == TTLocalizer.OptionsPageTitle:
            ...
            iconModels.detachNode()
        elif pageName == TTLocalizer.EstatePageTitle:
            cardMaker = CardMaker('mapIcon')
            cardMaker.setFrameFullscreenQuad()
            iconGeom = iconModels = NodePath(cardMaker.generate())
            iconGeom.setTexture(loader.loadTexture('phase_3/maps/estate_map_icon.png'), 1)
            iconGeom.setTransparency(True)
            iconScale = 0.5
            iconModels.detachNode()
            ...
            iconImage = iconGeom = iconModels.find('**/card_back')
            iconScale = 0.22
            iconModels.detachNode()
            ...
            iconModels.detachNode()
        elif pageName == TTLocalizer.EffectsPageTitle:
            iconModels = loader.loadModel('phase_3/models/props/gm_icons')
            iconGeom = iconModels.find('**/access_level_400')
            iconScale = 0.75
            iconModels.detachNode()
            ...
            iconColor = (0, 0.6, 1, 1)
            iconModels.detachNode()
        elif pageName == TTLocalizer.BadgePageTitle:
            iconModels = loader.loadModel('phase_3.5/models/gui/tt_m_gui_gen_emblemIcons')
            iconGeom = iconModels.find('**/tt_t_gui_gen_emblemGold')
            iconModels.detachNode()
        if pageName == TTLocalizer.OptionsPageTitle:
            pageName = TTLocalizer.OptionsTabTitle
        rightSide = pageIndex < 16
        pageTab = DirectButton(parent=self.pageTabFrames[rightSide], relief=DGG.RAISED, frameSize=(-0.575, 0.575, -0.575, 0.575), borderWidth=(0.05, 0.05), text=('',
         '',
         pageName,
         ''), text_align=TextNode.ALeft if rightSide else TextNode.ARight, text_pos=(1 if rightSide else -1.1, -0.2), text_scale=TTLocalizer.SBpageTab, text_fg=(1, 1, 1, 1), text_shadow=(0, 0, 0, 1), image=iconImage, image_scale=iconScale, geom=iconGeom, geom_scale=iconScale, geom_color=iconColor, pos=(0, 0, -yOffset), scale=0.06, command=buttonPressedCommand, extraArgs=extraArgs)
