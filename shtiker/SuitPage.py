# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.shtiker.SuitPage
...
class SuitPage(ShtikerPage.ShtikerPage):
    ...
    def load(self):
        ...
        self.frameNode.setScale(0.8, 1, 0.98)
        ...
        frameModel.setScale(0.0254, 0.03, 0.045)
        frameModel.setPos(0, 20, -0.573)
        frame = frameModel.find('**/frame')
        frame.wrtReparentTo(self.frameNode)
        frame.setScale(0.045)
        self.frameNode.setPos(-0.012, 0, 0)
        icons = frameModel.find('**/icons')
        del frameModel
        self.title = DirectLabel(parent=self.iconNode, relief=None, text=TTLocalizer.SuitPageTitle, text_scale=0.08, text_pos=(0.015, 0))
        self.radarButtons = []
        icon = icons.find('**/corp_icon')
        self.corpRadarButton = DirectButton(parent=self.iconNode, relief=None, state=DGG.DISABLED, image=icon, image_scale=(0.03375, 1, 0.045), image2_color=Vec4(1.0, 1.0, 1.0, 0.75), pos=(-0.13, 0, -0.5), scale=0.9, command=self.toggleRadar, extraArgs=[0])
        self.radarButtons.append(self.corpRadarButton)
        icon = icons.find('**/legal_icon')
        self.legalRadarButton = DirectButton(parent=self.iconNode, relief=None, state=DGG.DISABLED, image=icon, image_scale=(0.03375, 1, 0.045), image2_color=Vec4(1.0, 1.0, 1.0, 0.75), pos=(-0.13, 0, -0.47), scale=0.9, command=self.toggleRadar, extraArgs=[1])
        self.radarButtons.append(self.legalRadarButton)
        icon = icons.find('**/money_icon')
        self.moneyRadarButton = DirectButton(parent=self.iconNode, relief=None, state=DGG.DISABLED, image=(icon, icon, icon), image_scale=(0.03375, 1, 0.045), image2_color=Vec4(1.0, 1.0, 1.0, 0.75), pos=(-0.13, 0, -0.43), scale=0.9, command=self.toggleRadar, extraArgs=[2])
        self.radarButtons.append(self.moneyRadarButton)
        icon = icons.find('**/sales_icon')
        self.salesRadarButton = DirectButton(parent=self.iconNode, relief=None, state=DGG.DISABLED, image=(icon, icon, icon), image_scale=(0.03375, 1, 0.045), image2_color=Vec4(1.0, 1.0, 1.0, 0.75), pos=(-0.13, 0, -0.39), scale=0.9, command=self.toggleRadar, extraArgs=[3])
        self.radarButtons.append(self.salesRadarButton)
        self.techRadarButton = DirectButton(parent=self.iconNode, relief=None, state=DGG.DISABLED, image='phase_3/maps/techbot_tab.png', image_scale=(0.03375, 1, 0.045), image2_color=Vec4(1.0, 1.0, 1.0, 0.75), pos=(-0.669, 0, -1.055), scale=(2, 1, 2.7), command=self.toggleRadar, extraArgs=[4])
        self.techRadarButton.setTransparency(True)
        ...
        self.panelModel = loader.loadModel('phase_3.5/models/gui/suitpage_card')
        self.makePanels()
        self.radarOn = [0] * len(SuitDNA.suitDepts)
        priceScale = 0.1
    ...
    def makePanels(self):
        ...
        gui.find('**/avatar_panel/shadow').removeNode()
        self.rolloverFrame = DirectFrame(self.panelNode, relief=None, geom=gui.find('**/avatar_panel'), geom_color=(0.5, 0.5, 0.5, 1), scale=1.2)
        self.rolloverFrame.setBin('gui-popup', 0)
        self.rolloverFrame.hide()
        self.rolloverLabel = DirectLabel(self.rolloverFrame, relief=None, text_scale=0.06, text='', text_fg=(1, 1, 1, 1), text_font=ToontownGlobals.getSuitFont(), text_wordwrap=17.5)
        gui.removeNode()
        for dept in xrange(0, len(SuitDNA.suitDepts)):
            color = PANEL_COLORS[dept]
            for type in xrange(0, SuitDNA.suitsPerDept):
                panel = DirectLabel(parent=self.panelNode, pos=(xStart + type * xOffset, 0.0, yStart - dept * yOffset), relief=None, state=DGG.NORMAL, image=self.panelModel, image_scale=(1, 1, 1), image_color=color, text=TTLocalizer.SuitPageMystery, text_scale=0.048, text_fg=(0, 0, 0, 1), text_pos=(0, 0.185, 0), text_font=ToontownGlobals.getSuitFont(), text_wordwrap=7)
                panel.setScale(0.6)
    ...
    def showInfo(self, panel, text, extra):
        ...
        self.rolloverFrame['geom_scale'] = Vec3(xAxis / 2 + 0.075, 1, yAxis / 3.5)
        self.rolloverLabel.setPos(0, 0, yAxis / 2 - 0.035)
        self.rolloverFrame.setPos(abs(left) * 1.25 + 0.29, 0, 0)
    ...
    def hideInfo(self, extra):
        if not self.currentPanel:
            ...
            self.currentPanel.setScale(0.6)
            self.currentPanel.clearBin()
            self.currentPanel = None
            self.rolloverFrame.hide()
            return
    ...
    def addQuotaLabel(self, panel):
        ...
        quotaLabel = DirectLabel(parent=panel, pos=(0.0, 0.0, -0.215), relief=None, state=DGG.DISABLED, text=TTLocalizer.SuitPageQuota % (count, quota), text_scale=0.06, text_fg=(0, 0, 0, 1), text_font=ToontownGlobals.getSuitFont())
    ...
    def addCogRadarLabel(self, panel):
        cogRadarLabel = DirectLabel(parent=panel, pos=(0.0, 0.0, -0.215), relief=None, state=DGG.DISABLED, text='', text_scale=0.048, text_fg=(0, 0, 0, 1), text_font=ToontownGlobals.getSuitFont())
        panel.cogRadarLabel = cogRadarLabel
        return

    def addSummonButton(self, panel):
        panel.summonButton = DirectButton(parent=panel, pos=(0.1, 0.0, -0.13), scale=0.1, relief=None, state=DGG.NORMAL, image=Preloaded['okButton'], image_scale=13.0, geom=Preloaded['evidence'], geom_scale=0.7, text=('',
    ...
    def resetPanel(self, dept, type):
        ...
        if panel.summonButton:
            panel.summonButton.hide()
        self.rolloverFrame.hide()
        panel.unbind(DGG.ENTER)
        panel.unbind(DGG.EXIT)
        color = PANEL_COLORS[dept]
    ...
    def setPanelStatus(self, panel, status):
        ...
        if status in (COG_DEFEATED, COG_COMPLETE1, COG_COMPLETE2):
            ...
            motto = TTLocalizer.BattleCogPopupWeaknesses[name]
            info = TTLocalizer.SuitPageAttackFormat % (level + 1,
             level + 5,
             self.getAttackStrings(groupAttacks),
             self.getAttackStrings(singleAttacks),
             motto)
            panel.bind(DGG.ENTER, self.showInfo, extraArgs=[panel, info])
            panel.bind(DGG.EXIT, self.hideInfo)
    ...
    def updateCogRadar(self, deptNum, panels):
        if hasattr(base.cr, 'currSuitPlanner') and base.cr.currSuitPlanner != None:
    ...
    def updateBuildingRadar(self, deptNum):
        if hasattr(base.cr, 'currSuitPlanner') and base.cr.currSuitPlanner != None:
        ...
        if button.building:
            ...
            if self.radarOn[deptNum]:
                dept = ord(SuitDNA.suitDepts[deptNum])
                num = len([ True for building in buildingList if building[1] == dept ])
                if num == 1:
