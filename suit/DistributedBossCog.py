# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.suit.DistributedBossCog
from panda3d.core import BitMask32, CSDefault, CollideMask, CollisionHandler, CollisionHandlerFloor, CollisionHandlerPusher, CollisionHandlerQueue, CollisionNode, CollisionPolygon, CollisionRay, CollisionSphere, CollisionTube, GeomNode, Mat3, Mat4, NodePath, Point3, VBase3, VBase4, Vec2, Vec3, composeMatrix, decomposeMatrix, headsUp, lookAt
...
class DistributedBossCog(DistributedAvatar.DistributedAvatar, BossCog.BossCog):
    ...
    def destroySkipButton(self):
        if self.skipSequence:
            self.skipSequence.pause()
            self.skipSequence = None
        if self.skipButton:
            self.skipButton.destroy()
            self.skipButton = None
            base.setMouseWatch(False)
            self.ignore('touchedTop')
            self.ignore('leftTop')
        return

    def setSkipStatus(self, skippable, current, all):
        if not skippable:
            self.destroySkipButton()
            return
        else:
            if not self.skipButton:
                self.skipButton = DirectButton(base.a2dTopCenter, relief=None, state=DGG.NORMAL, image=Preloaded['blueButton'], image_color=(0.3, 0.3, 0.3, 1), pos=(0, 0, 0.2), image_scale=(1.7, 1, 1.5), text='', text_fg=(1, 1, 1, 1), text_shadow=(0, 0, 0, 1), text_scale=0.1, text_font=ToontownGlobals.getSuitFont(), text_roll=-1, text_pos=(0, 0.03), command=self.__requestSkip)
                self.skipButton.setBin('gui-popup', 0)
                self.skipButton.hide()
                base.setMouseWatch(True)
                self.accept('touchedTop', self.__touchedTop)
                self.accept('leftTop', self.__leftTop)
            self.skipButton['text'] = TTLocalizer.SkipCutsceneText % (current, all)
            if current == all:
                self.finishAllMovies()
            return

    def __touchedTop(self):
        if self.skipSequence:
            self.skipSequence.pause()
        self.skipSequence = Sequence(Func(self.skipButton.show), self.skipButton.posInterval(0.5, (0, 0, -0.2), self.skipButton.getPos(), blendType='easeInOut'))
        self.skipSequence.start()

    def __leftTop(self):
        if self.skipSequence:
            self.skipSequence.pause()
        self.skipSequence = Sequence(self.skipButton.posInterval(0.5, (0, 0, 0.2), self.skipButton.getPos(), blendType='easeInOut'), Func(self.skipButton.hide))
        self.skipSequence.start()

    def __requestSkip(self):
        self.d_requestSkip()
    ...
    def finishAllMovies(self):
        finished = False
        for interval in self.activeIntervals.keys():
            if interval.endswith('Movie'):
                self.clearInterval(interval, True)
                finished = True
    ...
    def toonNormalEyes(self, toons):
        sequence = Sequence()
        for toon in toons:
            toon = self.cr.doId2do.get(toon)
            if toon:
                sequence.append(Func(toon.normalEyes))
                sequence.append(Func(toon.blinkEyes))
    ...
    def loadEnvironment(self):
        ...
        self.stingMusic = loader.loadMusic('phase_7/audio/bgm/encntr_suit_winning_indoor.ogg')
        self.battleOneMusic = loader.loadMusic('phase_3.5/audio/bgm/encntr_general_bg.ogg')
        self.battleThreeMusic = loader.loadMusic('phase_7/audio/bgm/encntr_suit_winning_indoor.ogg')
        self.epilogueMusic = loader.loadMusic('phase_9/audio/bgm/encntr_hall_of_fame.ogg')
