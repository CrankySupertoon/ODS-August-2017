# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.suit.DistributedBossbotBoss
from panda3d.core import BitMask32, BoundingSphere, CSDefault, CollideMask, CollisionHandler, CollisionHandlerEvent, CollisionNode, CollisionPlane, CollisionSphere, CollisionTube, GeomNode, Lens, Mat3, NodePath, Plane, Point3, Texture, TextureStage, VBase3, VBase4, Vec2, Vec3, composeMatrix, decomposeMatrix, headsUp, lookAt
...
class DistributedBossbotBoss(DistributedBossCog.DistributedBossCog, FSM.FSM):
    ...
    def loadEnvironment(self):
        ...
        self.betweenPhaseMusic = loader.loadMusic('phase_9/audio/bgm/encntr_toon_winning.ogg')
        self.phaseTwoMusic = loader.loadMusic('phase_12/audio/bgm/BossBot_CEO_v1.ogg')
        self.phaseFourMusic = loader.loadMusic('phase_12/audio/bgm/BossBot_CEO_v2.ogg')
    ...
    def enterBattleTwo(self):
        ...
        seq = self.createTalkSequence(TTLocalizer.CEOSpeech, 10)
        seq.start()
        self.storeInterval(seq, intervalName)
    ...
    def __hpChange(self, *args):
        hasLaff = self.hasRequiredLaff(base.localAvatar)
        for belt in self.belts:
            if belt:
                belt.textPath.show() if hasLaff else belt.textPath.hide()

    def hasRequiredLaff(self, av):
        return av.getHp() >= int(math.ceil(av.getMaxHp() * 0.43))

    def localToonTouchedBeltFood(self, beltIndex, foodIndex, foodNum):
        avId = base.localAvatar.doId
        if avId not in self.toonFoodStatus or not self.toonFoodStatus[avId]:
    ...
    def enterBattleFour(self):
        ...
        self.accept(base.localAvatar.uniqueName('hpChange'), self.__hpChange)
        self.__hpChange()
        base.playMusic(self.phaseFourMusic, looping=1, volume=0.9)

    def exitBattleFour(self):
        DistributedBossCog.DistributedBossCog.exitBattleFour(self)
        self.ignore(base.localAvatar.uniqueName('hpChange'))
        self.phaseFourMusic.stop()
    ...
    def enterEpilogue(self):
        ...
        seq.start()
        self.storeInterval(seq, intervalName)
        self.acceptOnce(self.resistanceToon.uniqueName('doneChatPage'), self.__doneEpilogue)
    ...
    def __doneEpilogue(self, elapsedTime = 0):
        ...
        track.start()
        self.ignore(self.resistanceToon.uniqueName('doneChatPage'))
    ...
    def zapLocalToon(self, attackCode, origin = None):
        if self.localToonIsSafe or localAvatar.ghostMode or localAvatar.isStunned:
            return
        elif globalClock.getFrameTime() < self.lastZapLocalTime + 1.0:
            return
        else:
    ...
    def localToonTouchedBeltToonup(self, beltIndex, toonupIndex, toonupNum):
        if not self.hasRequiredLaff(base.localAvatar):
