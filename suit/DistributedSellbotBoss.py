# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.suit.DistributedSellbotBoss
from panda3d.core import CollideMask, CollisionNode, CollisionPolygon, CollisionTube, GeomNode, Lens, NodePath, Point3, RopeNode, TextNode, Texture, VBase3, VBase4, Vec3, headsUp
...
class DistributedSellbotBoss(DistributedBossCog.DistributedBossCog, FSM.FSM):
    ...
    def handlePracticeStart(self, toon):
        toon.setPosHpr(-0.09, 77.25, 17.95, 180, 0, 0)
    ...
    def setDoobers(self, dooberDna):
        self.dooberDna = dooberDna

    def getDoobers(self):
        return self.dooberDna
    ...
    def __walkDoober(self, suit, delay, turnPos, track):
        turnPos = Point3(*turnPos)
        turnPosDown = Point3(*ToontownGlobals.SellbotBossDooberTurnPosDown)
        turnPosDown2 = Point3(*ToontownGlobals.SellbotBossDooberTurnPosDown2)
        turnPosDown3 = Point3(*ToontownGlobals.SellbotBossDooberTurnPosDown3)
        flyPos = Point3(*ToontownGlobals.SellbotBossDooberFlyPos)
        seq = Sequence(Func(suit.headsUp, turnPos), Wait(delay), Func(suit.loop, 'walk', 0), self.__walkSuitToPoint(suit, suit.getPos(), turnPos), self.__walkSuitToPoint(suit, turnPos, turnPosDown), self.__walkSuitToPoint(suit, turnPosDown, turnPosDown2), self.__walkSuitToPoint(suit, turnPosDown2, turnPosDown3), self.__walkSuitToPoint(suit, turnPosDown3, flyPos), suit.beginSupaFlyMove(flyPos, 0, 'flyAway', skyPosCurrent=False), Func(suit.delete))
        track.append(seq)
    ...
    def makeIntroductionMovie(self, delayDeletes):
        ...
        if not self.doobers:
            for dna in self.dooberDna:
                dna = SuitDNA.SuitDNA(dna)
                suit = DistributedSuitBase(self.cr)
                suit.doId = id(suit)
                suit.setDNA(dna)
        ...
        self.__walkDoober(self.doobers[0], 0, turnPosA, dooberTrack)
        self.__walkDoober(self.doobers[1], 2, turnPosA, dooberTrack)
        self.__walkDoober(self.doobers[2], 6, turnPosA, dooberTrack)
        self.__walkDoober(self.doobers[3], 10, turnPosA, dooberTrack)
        self.__walkDoober(self.doobers[7], 1, turnPosB, dooberTrack)
        self.__walkDoober(self.doobers[6], 4, turnPosB, dooberTrack)
        self.__walkDoober(self.doobers[5], 8, turnPosB, dooberTrack)
        self.__walkDoober(self.doobers[4], 12, turnPosB, dooberTrack)
        toonTrack = Parallel()
    ...
    def loadEnvironment(self):
        ...
        self.promotionMusic = loader.loadMusic('phase_7/audio/bgm/encntr_suit_winning_indoor.ogg')
        self.betweenBattleMusic = loader.loadMusic('phase_9/audio/bgm/encntr_toon_winning.ogg')
        self.battleTwoMusic = loader.loadMusic('phase_7/audio/bgm/encntr_suit_winning_indoor.ogg')
        self.battleThreeMusic = loader.loadMusic('phase_7/audio/bgm/encntr_suit_winning_indoor.ogg')
    ...
    def enterPrepareBattleTwo(self):
        ...
        (localAvatar.setCameraFov(ToontownGlobals.CogHQCameraFov),)
        self.hide()
        self.acceptOnce(self.cagedToon.uniqueName('doneChatPage'), self.__onToBattleTwo)
    ...
    def exitPrepareBattleTwo(self):
        ...
        self.ignore(self.cagedToon.uniqueName('doneChatPage'))
        self.__clearOnscreenMessage()
        self.stingMusic.stop()
    ...
    def enterPrepareBattleThree(self):
        ...
        (localAvatar.setCameraFov(ToontownGlobals.CogHQCameraFov),)
        self.hide()
        self.acceptOnce(self.cagedToon.uniqueName('doneChatPage'), self.__onToBattleThree)
        self.cagedToon.setLocalPageChat(TTLocalizer.CagedToonPrepareBattleThree, 1)
        base.playMusic(self.betweenBattleMusic, looping=1, volume=0.9)

    def __onToBattleThree(self, elapsed = 0):
    ...
    def enterEpilogue(self):
        ...
        seq.start()
        self.storeInterval(seq, intervalName)
        self.accept(self.cagedToon.uniqueName('nextChatPage'), self.__epilogueChatNext)
        self.accept(self.cagedToon.uniqueName('doneChatPage'), self.__epilogueChatDone)
    ...
    def __epilogueChatDone(self, elapsed):
        ...
        self.ignore(self.cagedToon.uniqueName('nextChatPage'))
        self.ignore(self.cagedToon.uniqueName('doneChatPage'))
        intervalName = 'EpilogueMovieToonAnim'
        self.clearInterval(intervalName)
    ...
    def __doobersToPromotionPosition(self, doobers, battleNode):
        ...
        for i, suit in enumerate(doobers):
            suit.loop('neutral')
            pos, h = points[i]
