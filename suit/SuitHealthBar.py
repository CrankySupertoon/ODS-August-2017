# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.suit.SuitHealthBar
from panda3d.core import Light, Vec4
from direct.task.Task import Task
from toontown.battle import BattleProps
HEALTH_COLORS = (Vec4(0, 1, 0, 1),
 Vec4(0.5, 1, 0, 1),
 Vec4(0.75, 1, 0, 1),
 Vec4(1, 1, 0, 1),
 Vec4(1, 0.86, 0, 1),
 ...
 Vec4(0.3, 0.3, 0.3, 1),
 Vec4(0.3, 0.3, 0.3, 1))
HEALTH_GLOW_COLORS = ((0.25, 1, 0.25, 0.5),
 (0.5, 1, 0.25, 0.5),
 (0.75, 1, 0.25, 0.5),
 (1, 1, 0.25, 0.5),
 (1, 0.866, 0.25, 0.5),
...
class SuitHealthBar:
    ...
    def getBlinkTaskName(self):
        return 'blink-task-%s' % id(self)

    def update(self, hp, forceUpdate = 0):
        if not self.geom:
            return
        condition = getHealthCondition(hp)
        if self.healthCondition != condition or forceUpdate:
            taskMgr.remove(self.getBlinkTaskName())
            if condition in (9, 10):
                ...
                taskMgr.add(blinkTask, self.getBlinkTaskName())
            else:
