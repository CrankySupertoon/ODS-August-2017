# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.toon.AvatarPanelBase
...
class AvatarPanelBase(AvatarPanel.AvatarPanel):
    ...
    def handleIgnoreConfirm(self, value):
        if value == -1:
            ...
            base.localAvatar.ignoreToon(self.avId, True)
    ...
    def handleStopIgnoringConfirm(self, value):
        if value == -1:
            ...
            base.localAvatar.ignoreToon(self.avId, False)
    ...
    def chooseReportCategory(self):
        ...
        DirectButton(parent=self.dialog, relief=None, image=yellowButton, image_scale=(2.125, 1.0, 1.0), text=TTLocalizer.ReportPanelCategoryLanguage, text_scale=0.06, text_pos=(0, -0.0124), pos=(0, 0, -0.3), command=self.handleReportCategory, extraArgs=[0])
        DirectButton(parent=self.dialog, relief=None, image=yellowButton, image_scale=(2.15, 1.0, 1.0), text=TTLocalizer.ReportPanelCategoryGreening, text_scale=0.06, text_pos=(0, -0.0125), pos=(0, 0, -0.425), command=self.handleReportCategory, extraArgs=[1])
        DirectButton(parent=self.dialog, relief=None, image=yellowButton, image_scale=(2.125, 1.0, 1.0), text=TTLocalizer.ReportPanelCategoryRude, text_scale=0.06, text_pos=(0, -0.0125), pos=(0, 0, -0.55), command=self.handleReportCategory, extraArgs=[2])
        DirectButton(parent=self.dialog, relief=None, image=yellowButton, image_scale=(2.125, 1.0, 1.0), text=TTLocalizer.ReportPanelCategoryName, text_scale=0.06, text_pos=(0, -0.0125), pos=(0, 0, -0.675), command=self.handleReportCategory, extraArgs=[3])
        DirectButton(parent=self.dialog, relief=None, image=yellowButton, image_scale=(2.125, 1.0, 1.0), text=TTLocalizer.ReportPanelCategoryHacking, text_scale=0.06, text_pos=(0, -0.0125), pos=(0, 0, -0.8), command=self.handleReportCategory, extraArgs=[4])
        self.dialog.show()
        self.__acceptStoppedStateMsg()
        self.requestStopped()
        return
    ...
    def handleReportCategory(self, value):
        self.cleanupDialog()
        if value >= 0 and OTPGlobals.isReportCategoryIndex(value):
            self.category = OTPGlobals.getReportCategory(value)
    ...
    def handleReportCategoryConfirm(self, value):
        ...
        if value > 0:
            if base.cr.isFriend(self.avId):
                ...
                removed = 1
            base.localAvatar.addReport(self.avId)
            base.cr.miscManager.sendReport(self.avId, self.category)
