# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.toon.DistributedNPCTailor
...
class DistributedNPCTailor(DistributedNPCToonBase):
    ...
    def __enterConfirmLoss(self, finished, which):
        if not self.popupInfo:
            self.popupInfo = DirectFrame(parent=hidden, relief=None, state='normal', text=TTLocalizer.STOREOWNER_CONFIRM_LOSS, text_wordwrap=10, frameSize=(-1, 1, -1, 1), text_pos=(0, -0.05), geom=DGG.getDefaultDialogGeom(), geom_color=ToontownGlobals.GlobalDialogColor, geom_scale=(0.88, 1, 0.55), geom_pos=(0, 0, -0.18), text_scale=0.08)
            DirectButton(self.popupInfo, image=Preloaded['okButton'], relief=None, text=TTLocalizer.STOREOWNER_OK, text_scale=0.05, text_pos=(0.0, -0.1), pos=(-0.08, 0.0, -0.31), command=self.__handleConfirmLossOK, extraArgs=[finished, which])
            DirectButton(self.popupInfo, image=Preloaded['closeButton'], relief=None, text=TTLocalizer.STOREOWNER_CANCEL, text_scale=0.05, text_pos=(0.0, -0.1), pos=(0.08, 0.0, -0.31), command=self.__handleConfirmLossCancel)
