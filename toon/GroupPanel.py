# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.toon.GroupPanel
...
class GroupPanel(DirectObject.DirectObject):
    ...
    def __init__(self, boardingParty):
        ...
        self.practiceFrame = None
        self.practiceLabel = None
    ...
    def cleanup(self):
        ...
        if self.practiceFrame:
            self.practiceFrame.destroy()
        if self.practiceLabel:
            self.practiceLabel.destroy()
    ...
    def __togglePractice(self):
        if self.practiceCooldown > time.time():
            return
        self.setPractice(not self.practice)
        self.boardingParty.requestPractice(self.practice)
        self.practiceCooldown = time.time() + 0.5

    def setPractice(self, practice):
        self.practice = practice
        if self.practiceButton:
            if self.practice:
                self.practiceButton['image_color'] = (0, 1, 0, 1)
            else:
                self.practiceButton['image_color'] = (1, 0, 0, 1)
    ...
    def __load(self):
        ...
        if self.boardingParty.getPracticeEnabled():
            self.practiceFrame = DirectFrame(self.frame, relief=None, image=Preloaded['squareBox'], image_scale=(0.56, 1, 0.1), pos=(0, 0, 0.415), image_color=(0.96, 0.43, 0.1, 1))
            self.practiceLabel = DirectLabel(self.practiceFrame, relief=None, text=TTLocalizer.ElevatorPractice, text_scale=0.07, pos=(-0.05, 0, -0.02), text_font=ToontownGlobals.getToonFont())
            self.practiceButton = DirectButton(self.practiceLabel, relief=None, image=Preloaded['circleButton'], image_color=(1, 0, 0, 1), scale=0.4, pos=(0.18, 0, 0.0175), command=self.__togglePractice, state=DGG.NORMAL)
            if localAvatar.doId != self.leaderId:
                self.practiceButton['state'] = DGG.DISABLED
