# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.toon.InventoryNew
from panda3d.core import Point4, TextEncoder, TextNode, Vec4
...
class InventoryNew(InventoryBase.InventoryBase, DirectFrame):
    ...
    WeaknessPressableImageColor = Vec4(0.4, 1, 0.94, 1)
    PropBonusNoncreditPressableImageColor = Vec4(0.6, 0.6, 0.3, 1)
    WeaknessNoncreditPressableImageColor = Vec4(0.28, 0.77, 0.73, 1)
    ...
    def __init__(self, toon, invStr = None, ShowSuperGags = 1):
        ...
        self.__battleCreditMultiplier = 1.0
    ...
    def getRealBattleCreditMultiplier(self):
        multiplier = self.getBattleCreditMultiplier()
        if base.cr.newsManager.isHolidayRunning(ToontownGlobals.MORE_XP_HOLIDAY) or base.localAvatar.hasMoreXp():
            multiplier += 1.0
        return multiplier
    ...
    def unload(self):
        ...
        self.invFrame.removeNode()
        self.detailFrame.removeNode()
    ...
    def load(self):
        ...
        self.buttonTuple = (self.upButton, self.downButton, self.rolloverButton)
        ...
        self.deleteEnterButton = DirectButton(parent=self.invFrame, image=Preloaded['trashcan'], text=('', TTLocalizer.InventoryDelete, TTLocalizer.InventoryDelete), text_fg=(1, 1, 1, 1), text_shadow=(0, 0, 0, 1), text_scale=0.1, text_pos=(0, -0.1), text_font=getInterfaceFont(), textMayChange=0, relief=None, pos=(-1, 0, -0.35), scale=1.0)
        self.deleteAllButton = DirectButton(parent=self.invFrame, image=Preloaded['trashcan'], text=('', TTLocalizer.InventoryDeleteAll, TTLocalizer.InventoryDeleteAll), text_fg=(1, 0, 0, 1), text_shadow=(1, 1, 1, 1), text_scale=0.1, text_pos=(0, -0.1), text_font=getInterfaceFont(), textMayChange=0, relief=None, pos=(-0.3, 0, -0.91), scale=0.75, command=self.__zeroInvConfirm)
        self.deleteExitButton = DirectButton(parent=self.invFrame, image=Preloaded['trashcan'], text=('', TTLocalizer.InventoryDone, TTLocalizer.InventoryDone), text_fg=(1, 1, 1, 1), text_shadow=(0, 0, 0, 1), text_scale=0.1, text_pos=(0, -0.1), text_font=getInterfaceFont(), textMayChange=0, relief=None, pos=(-1, 0, -0.35), scale=1.0)
        ...
        self.detailDataLabel = DirectLabel(parent=self.detailFrame, text='', text_fg=(0.05, 0.14, 0.4, 1), scale=0.04, pos=(-0.22, 0, -0.22), text_font=getInterfaceFont(), text_align=TextNode.ALeft, relief=None)
    ...
    def showDetail(self, track, level, event = None):
        ...
        if self.itemIsCredit(track, level):
            mult = self.getRealBattleCreditMultiplier()
            credit = (level + 1) * mult
            if self.toon.earnedExperience:
                maxCredit = ExperienceCap - self.toon.earnedExperience[track]
                credit = min(credit, maxCredit)
            skillCredit = TTLocalizer.InventorySkillCredit % ('%g' % credit)
        else:
            skillCredit = TTLocalizer.InventorySkillCreditNone
        gagTuple = (track, level)
        if gagTuple in SuitGlobals.WeaknessToSuit:
            vulnerable = [ SuitBattleGlobals.SuitAttributes[suit]['pluralname'] for suit in SuitGlobals.WeaknessToSuit[gagTuple] ]
            vulnerableCogs = TTLocalizer.InventoryVulnerableCogs % (', '.join(vulnerable), getWeaknessBonus(damage))
        else:
            vulnerableCogs = TTLocalizer.InventoryVulnerableCogsNone
        text = TTLocalizer.InventoryDetailData % {'accuracy': accString,
         'damageString': self.getToonupDmgStr(track, level),
         'damage': damage,
         'bonus': damageBonusStr,
         'singleOrGroup': self.getSingleGroupStr(track, level),
         'skillCredit': skillCredit,
         'vulnerableCogs': vulnerableCogs}
        self.detailDataLabel.configure(text=text)
    ...
    def setActivateMode(self, mode, heal = 1, trap = 1, lure = 1, bldg = 0, creditLevel = None, gagTutMode = 0):
    ...
    def battleActivateButtons(self):
        ...
        if self.bldg == 1:
            ...
            self.hideTutorialFrame()
        elif self.isTutorial():
            if not self.tutorialFrame:
                self.loadTutorialFrame()
    ...
    def makePressable(self, button, track, level):
        organicBonus = self.toon.checkGagBonus(track, level)
        propBonus = self.checkPropBonus(track)
        weaknessBonus = self.checkWeaknessBonus(track, level)
        bonus = organicBonus or propBonus or weaknessBonus
        if bonus:
        ...
        if propBonus:
            ...
            button.configure(image_color=self.WeaknessPressableImageColor)
            self.addToPropBonusIval(button)
        else:
    ...
    def makeNoncreditPressable(self, button, track, level):
        ...
        if propBonus:
            ...
            button.configure(image_color=self.WeaknessNoncreditPressableImageColor)
            self.addToPropBonusIval(button)
        else:
    ...
    def __makeTutorialDialog(self, parent, pos, text_scale = 0.07, text = ''):
        return DirectFrame(parent, relief=None, geom=DGG.getDefaultDialogGeom(), geom_color=ToontownGlobals.GlobalDialogColor, geom_scale=(1, 1, 0.5), pos=pos, text=text, text_scale=text_scale, text_wordwrap=16, text_pos=(0, 0.15))

    def __makeBattleButton(self, parent, pos, text, text_scale, command):
        return DirectButton(parent, relief=None, pos=pos, text=text, text_scale=text_scale, text_pos=(0, -0.02), text_fg=(1, 1, 1, 1), image=self.buttonTuple, image_scale=1.05, image_color=(0, 0.6, 1, 1), command=command)
    ...
    def loadBattleFrame(self):
        ...
        self.runButton = self.__makeBattleButton(self.battleFrame, (0.73, 0, -0.4), TTLocalizer.InventoryRun, TTLocalizer.INrunButton, self.__handleRun)
        self.sosButton = self.__makeBattleButton(self.battleFrame, (0.96, 0, -0.4), TTLocalizer.InventorySOS, TTLocalizer.INrunButton, self.__handleSOS)
        self.passButton = self.__makeBattleButton(self.battleFrame, (0.96, 0, -0.24), TTLocalizer.InventoryPass, TTLocalizer.INpassButton, self.__handlePass)
        self.fireButton = self.__makeBattleButton(self.battleFrame, (0.73, 0, -0.24), TTLocalizer.InventoryFire, TTLocalizer.INfireButton, self.__handleFire)
        battleModels.removeNode()
        self.battleFrame.hide()
        return

    def loadTutorialFrame(self):
        self.tutorialFrame = DirectFrame(self.battleFrame, pos=(0, 0, -0.8))
        tutorialChooseDialog = self.__makeTutorialDialog(self.tutorialFrame, (-0.6, 0, 0), text=TTLocalizer.InventoryDefeatHollywood)
        self.tutorialHelpDialog = self.__makeTutorialDialog(self.tutorialFrame, (0.63, 0, 0), 0.06)
        self.tutorialHelpDialog.hide()
        self.gagsHelpButton = self.__makeBattleButton(tutorialChooseDialog, (-0.35, 0, -0.125), TTLocalizer.InventoryGags, TTLocalizer.INrunButton, self.__openGagTutorial)
        self.fireHelpButton = self.__makeBattleButton(tutorialChooseDialog, (-0.0125, 0, -0.125), TTLocalizer.InventoryFire, TTLocalizer.INfireButton, self.__openFireTutorial)
        self.sosHelpButton = self.__makeBattleButton(tutorialChooseDialog, (0.325, 0, -0.125), TTLocalizer.InventorySOS, TTLocalizer.INrunButton, self.__openSOSTutorial)
        self.tutorialPulseSequence = None
        return

    def __stopPulse(self):
        if self.tutorialPulseSequence:
            self.tutorialPulseSequence.finish()
            self.tutorialPulseSequence = None
        return

    def __pulseItems(self, *items):
        self.__stopPulse()
        self.tutorialPulseSequence = Parallel(*[ Sequence(item.scaleInterval(0.5, 1.3, blendType='easeInOut'), item.scaleInterval(0.5, 1, blendType='easeInOut')) for item in items ])
        self.tutorialPulseSequence.loop()

    def __hideTutorialButton(self, index):
        for i, button in enumerate((self.gagsHelpButton, self.fireHelpButton, self.sosHelpButton)):
            button.hide() if i == index else button.show()

    def __openGagTutorial(self):
        self.__openTutorial(0, TTLocalizer.InventoryGagsHelp, self.buttons[ToontownBattleGlobals.LURE_TRACK][4], self.buttons[ToontownBattleGlobals.THROW_TRACK][5])

    def __openFireTutorial(self):
        self.__openTutorial(1, TTLocalizer.InventoryFireHelp, self.fireButton)

    def __openSOSTutorial(self):
        self.__openTutorial(2, TTLocalizer.InventorySOSHelp, self.sosButton)

    def __openTutorial(self, index, text, *items):
        self.__hideTutorialButton(index)
        self.tutorialHelpDialog.show()
        self.tutorialHelpDialog['text'] = text
        self.__pulseItems(*items)

    def hideTutorialFrame(self):
        if not self.tutorialFrame:
            return
        else:
            self.__stopPulse()
    ...
    def checkWeaknessBonus(self, track, level):
        if not hasattr(base, 'townBattle') or not base.townBattle or not hasattr(base.townBattle, 'cogs'):
            return False
        gag = (track, level)
        if gag not in SuitGlobals.WeaknessToSuit:
            return False
        suits = SuitGlobals.WeaknessToSuit[gag]
        return any((True for suit in base.townBattle.cogs if suit.dna.name in suits))
