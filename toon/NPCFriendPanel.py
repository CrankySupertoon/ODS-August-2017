# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.toon.NPCFriendPanel
...
class NPCFriendPanel(DirectFrame):
    ...
    def __init__(self, parent = aspect2d, callable = False, **kw):
        ...
        self.leftArrow = DirectButton(parent=self, relief=None, image=Preloaded['slimBlueArrow'], pos=(-6.8, 0, 0), scale=3.8, command=self.addPageIndex, extraArgs=[-1])
        self.rightArrow = DirectButton(parent=self, relief=None, image=Preloaded['slimBlueArrow'], pos=(6.8, 0, 0), scale=-3.8, command=self.addPageIndex, extraArgs=[1])
        self.leftArrow.hide()
        return
...
class NPCFriendCard(DirectFrame):
    ...
    def __init__(self, parent = aspect2dp, **kw):
        ...
        self.front = DirectFrame(parent=self, relief=None, image=Preloaded['playCardFront'])
        self.front.hide()
        self.back = DirectFrame(parent=self, relief=None, image=Preloaded['playCard'])
        ...
        self.sosCallButton = DirectButton(parent=self.front, relief=None, text=TTLocalizer.NPCCallButtonLabel, text_fg=self.normalTextColor, text_scale=0.28, text_align=TextNode.ACenter, image=Preloaded['callButton'], image_color=(1.0, 0.2, 0.2, 1), image0_color=Vec4(1.0, 0.4, 0.4, 1), image3_color=Vec4(1.0, 0.4, 0.4, 0.4), image_scale=(4.4, 1, 3.6), image_pos=Vec3(0, 0, 0.08), pos=(-1.15, 0, -0.9), scale=1.25, command=self.__chooseNPCFriend)
        ...
        for i in xrange(self.maxRarity):
            label = DirectLabel(parent=self.front, relief=None, image=Preloaded['star'], image_scale=0.2, image_color=Vec4(0.502, 0.251, 0.251, 1.0), pos=(1.1 - i * 0.24, 0, -1.2))
            label.hide()
    ...
    def update(self, NPCID, count = 0, fCallable = 0):
        ...
        if fCallable:
            ...
            self.sosCountInfo['text_scale'] = 0.35
