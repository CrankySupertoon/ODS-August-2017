# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.toon.ToonAvatarDetailPanel
from panda3d.core import TextEncoder, TextNode, Vec3, Vec4
...
class ToonAvatarDetailPanel(DirectFrame):
    ...
    def __init__(self, avId, avName, parent = base.a2dTopRight, **kw):
        ...
        optiondefs = (('pos', (-0.79, 0.0, -0.47), None),
         ('scale', 0.5, None),
         ('relief', None, None),
         ('image', Preloaded['detailInfoPanel'], None),
        ...
        self.bCancel = DirectButton(self, image=Preloaded['closeButton'], image_scale=1.1, relief=None, text=TTLocalizer.AvatarDetailPanelCancel, text_scale=TTLocalizer.TADPbCancel, text_pos=(0.12, -0.01), pos=TTLocalizer.TADPbCancelPos, scale=2.0, command=self.__handleCancel)
        self.bCancel.hide()
        self.sosButton = DirectButton(self, relief=None, image=Preloaded['playCard'], scale=0.05, pos=(0.3, 0, -0.76), text=('',
         TTLocalizer.DetailPanelSOS,
         TTLocalizer.DetailPanelSOS,
         ''), text_fg=(1, 1, 0.5, 1), text_shadow=(0, 0, 0, 1), text_scale=2, text_pos=(0, -3.4), text_align=TextNode.ACenter, state=DGG.NORMAL, command=self.__toggleSOSGui)
        self.sosButton.hide()
        ...
        for text in xrange(0, 4):
            self.sosButton.component('text' + str(text)).setBin('gui-popup', 10)
    ...
    def __showData(self):
        ...
        identifier = TTLocalizer.getAvatarId(self.avId)
        if online:
            ...
            text = TTLocalizer.AvatarDetailPanelOffline % {'identifier': identifier,
             'lastSeen': TTLocalizer.getLastSeenString(av.getLastSeen())}
    ...
    def __updateTrackInfo(self):
        ...
        rolloverFrame = DirectFrame(self.inventoryFrame, relief=None, geom=DGG.getDefaultDialogGeom())
        rolloverFrame.setBin('gui-popup', 0)
        rolloverFrame.hide()
        rolloverLabel = DirectLabel(rolloverFrame, relief=None, text_scale=0.07, text='', text_fg=(1, 1, 1, 1))
                ...
                for item in xrange(0, len(Levels[track])):
                    ...
                    if curExp >= level:
                        ...
                        if numItems == 0:
                            ...
                            geom_color = None
                        pos = Vec3(xOffset + item * xSpacing, 0, yOffset + track * ySpacing)
                        ...
                        label.bind(DGG.ENTER, self.showInfo, extraArgs=[rolloverFrame,
                         rolloverLabel,
                         track,
                         int(getAvPropDamage(track, item, curExp, organic)),
                         numItems,
                         pos,
                         image_color])
    ...
    def showInfo(self, frame, label, track, damage, numItems, pos, color, extra):
        label['text'] = TTLocalizer.GagPopup % (self.avatar.inventory.getToonupDmgStr(track, 0), damage, numItems)
        label.resetFrameSize()
        left, right, down, up = label.node().getFrame()
        xAxis = abs(left) + abs(right)
        yAxis = abs(down) + abs(up)
        frame['geom_scale'] = (xAxis + 0.05, 0, yAxis + 0.04)
        frame['geom_color'] = color
        label.setPos(0, 0, yAxis / 2 - 0.05)
        frame.setPos(pos + Vec3(abs(left) + 0.125, 0, 0))
        frame.show()
    ...
    def __updateTrophyInfo(self):
        if self.createdAvatar:
            ...
            if self.avatar.trophyScore >= TrophyStarLevels[2]:
                ...
                color = None
            if color:
                self.star = DirectLabel(parent=self, image=Preloaded['detailStar'], image_color=color, pos=(0.610165, 0, -0.760678), scale=0.9, relief=None)
            return
    ...
    def __toggleSOSGui(self):
        ...
        if self.sosFrame.getScale() == 0.5:
            ...
            scale = 0.5
            self.sosFrame.setBin('background', 10)
        Sequence(Parallel(self.sosFrame.posInterval(1.0, pos, blendType='easeOut'), self.sosFrame.scaleInterval(1.0, scale, blendType='easeOut')), Func(self.__enableSOSButton)).start()
