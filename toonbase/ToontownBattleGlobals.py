# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.toonbase.ToontownBattleGlobals
...
SQUIRT_TRACK = 5
DROP_TRACK = 6
MAX_TRACK = DROP_TRACK
NPC_RESTOCK_GAGS = 7
NPC_TOONS_HIT = 8
NPC_COGS_MISS = 9
NPC_TOONS_POWER_UP = 10
NPC_COGS_POWER_DOWN = 11
MIN_TRACK_INDEX = 0
MAX_TRACK_INDEX = 6
MIN_LEVEL_INDEX = 0
MAX_LEVEL_INDEX = 6
...
TutorialLevels = [0,
 ...
 9768,
 7719]
EmptyLevels = [0] * len(Tracks)
TutorialInventory = [[0] * 7,
 [0,
  0,
  0,
  3,
...
COGS_POWER_DOWN = 8
TOONS_POWER_UP = 9
DeptToModifier = {'s': COGS_HIT,
 'm': TOONS_POWER_DOWN,
 'l': COGS_POWER_UP,
 'c': TOONS_MISS}

def getAvPropDamage(attackTrack, attackLevel, exp, organicBonus = False, propBonus = False, weaknessBonus = False):
    ...
    if propBonus:
        damage += getDamageBonus(originalDamage)
    if weaknessBonus:
        damage += getWeaknessBonus(originalDamage)
    return damage
...
def getWeaknessBonus(normal):
    return int(math.ceil(normal * 0.25))
...
def getMintCreditMultiplier(mintId):
    return {CashbotMintIntA: 2.0,
     CashbotMintIntB: 2.5,
     CashbotMintIntC: 3.0,
     CashbotMintIntD: 3.5}.get(mintId, 1.0)


def getServerRoomCreditMultiplier(serverRoomId):
    return {TechbotServerRoomIntA: 2.0,
     TechbotServerRoomIntB: 2.5,
     TechbotServerRoomIntC: 3.0}.get(serverRoomId, 1.0)
...
def decodeUber(flagMask):
    if not flagMask:
        ...
        return [ i for i in xrange(len(Levels)) if flagMask & 1 << i ]


def getUberFlag(flagMask, index):
    return index in decodeUber(flagMask)