# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.toontowngui.GUIUtils
...
def loadCircle():
    tex = loader.loadTexture('phase_3/maps/gui-circle.png')
    cm = CardMaker('circle')
    cm.setFrame(-1, 1, -1, 1)
...
def packText(text, name = 'textRoot', color = LVector4f(1, 1, 1, 1), font = None, scale = 1, stumble = 0, stomp = 0, wiggle = 0, kern = 0, indent = 0, height = 0, width = 0):
    ...
    if width != 0 and height != 0:
        for node in root.getChildren():
            theta = node.getX() / (height / 2.0) + indent * math.pi / 180
            d = node.getY()
            x = math.sin(theta) * (width / 2.0)
            y = (math.cos(theta) - 1) * (height / 2.0)
