# Fuck you Disyer. Stealing my fucking paypal. GET FUCKED: toontown.town.TownBattleToonPanel
from panda3d.core import TextEncoder, Texture, Vec4
...
class TownBattleToonPanel(DirectFrame):
    ...
    def __init__(self, battle):
        ...
        DirectFrame.__init__(self, relief=None, image=Preloaded['toonBattlePanel'], image_color=Vec4(0.5, 0.9, 0.5, 0.7))
        self.setScale(0.8)
    ...
    def setValues(self, index, track, level = None, numTargets = None, targetIndex = None, localNum = None):
        ...
        if track == BattleBase.NO_ATTACK or track == BattleBase.UN_ATTACK:
            ...
            info = TTLocalizer.BattleSOSPopup % (sosType,
             NPCToons.getNPCName(targetIndex),
             hpString if hp else '',
             rarity,
             count)
            self.hoverButton.bind(DGG.ENTER, self.battle.showRolloverFrame, extraArgs=[self, BattleHoverGag, info])
            ...
            if numTargets is not None and targetIndex is not None:
                self.whichText.show()
                self.whichText['text'] = self.determineWhichText(numTargets, targetIndex, index)
        else:
